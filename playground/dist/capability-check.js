...
const WASI_HOST = {
    ...
    reason: "The playground runs in a browser sandbox with no WASI runtime. " +
...
const FILE_IO = {
    ...
    reason: "The playground runs in a browser sandbox without file system access. " +
...
const NETWORK = {
    id: "network",
    name: "Network Access",
    reason: "The playground runs in a browser sandbox without direct network access. " +
...
const PROCESS_ENV = {
    ...
    reason: "The playground runs in a browser sandbox without access to " +
...
const DETECTION_RULES = [
    // --- File I/O (most specific first) ---
    {
        pattern: /\bstd::fs::\w+/g,
        capability: FILE_IO,
        messageTemplate: "'{match}' requires file system access, which is unavailable in the browser sandbox.",
    },
    {
    ...
    },
    {
        ...
        capability: FILE_IO,
        messageTemplate: "The 'fs' module requires file system access, which is unavailable in the browser sandbox.",
    },
    // --- Network ---
    {
        pattern: /\bstd::net::\w+/g,
        capability: NETWORK,
        messageTemplate: "'{match}' requires network access, which is unavailable in the browser sandbox.",
    },
    {
    ...
    },
    {
        ...
        capability: NETWORK,
        messageTemplate: "The 'net' module requires network access, which is unavailable in the browser sandbox.",
    },
    {
        pattern: /\bimport\s+http\b/g,
        capability: NETWORK,
        messageTemplate: "The 'http' module requires network access, which is unavailable in the browser sandbox.",
    },
    // --- Process environment ---
    {
        pattern: /\bstd::env::\w+/g,
        capability: PROCESS_ENV,
        messageTemplate: "'{match}' requires access to process environment variables, " +
    ...
    },
    {
        ...
        capability: PROCESS_ENV,
        messageTemplate: "The 'env' module requires access to process environment variables, " +
            "which is unavailable in the browser sandbox.",
    },
    // --- General WASI host (broadest patterns last) ---
    {
        ...
        messageTemplate: "'{match}' requires a WASI host runtime, which is unavailable in the browser sandbox.",
    },
    {
        pattern: /\bimport\s+host\b/g,
        capability: WASI_HOST,
        messageTemplate: "The 'host' module requires a WASI host runtime, which is unavailable in the browser sandbox.",
    },
];
// ---------------------------------------------------------------------------
// Detection logic
 ...
 * ```
 */
export function checkCapabilities(source) {
    if (source.length === 0) {
        return [];
    }
    const warnings = [];
 ...
 * ```
 */
export function capabilityWarningsToDiagnostics(warnings) {
    return warnings.map((w) => ({
        ...
        suggestion: "This code can be parsed, but cannot be " +
 ...
 */
export function getCapabilityInfo(id) {
    return UNSUPPORTED_CAPABILITIES.find((c) => c.id === id);
}
//# sourceMappingURL=capability-check.js.map