...
describe("ParseResponse", () => {
  it("accepts a successful parse response", () => {
    ...
    const resp = parseJson<ParseResponse>(json);
  ...
  it("handles multiple item kinds", () => {
    ...
    const resp = parseJson<ParseResponse>(json);
    const items: ModuleItem[] = resp.module!.items;
  ...
  it("handles import aliases", () => {
    ...
    const resp = parseJson<ParseResponse>(json);
    const imports: ModuleImport[] = resp.module!.imports;
...
describe("FormatResponse", () => {
  it("accepts a successful format response", () => {
    ...
    const resp = parseJson<FormatResponse>(json);
    assert.equal(resp.ok, true);
...
describe("TokenizeResponse", () => {
  it("accepts a successful tokenize response", () => {
    ...
    const resp = parseJson<TokenizeResponse>(json);
    assert.equal(resp.ok, true);
    assert.equal(resp.tokens.length, 4);

    const token: Token = resp.tokens[0];
...
describe("WorkerRequest", () => {
  it("validates init request shape", () => {
    const req: WorkerRequest = { id: 1, cmd: "init", wasmUrl: "/test.wasm" };
    assert.equal(req.cmd, "init");
    assert.equal(req.id, 1);
  });

  it("validates parse request shape", () => {
    const req: WorkerRequest = { id: 2, cmd: "parse", source: "fn main() {}" };
...
describe("WorkerResponse", () => {
  it("validates success response", () => {
    const resp: WorkerResponse = { id: 1, ok: true, result: { ok: true } };
    assert.equal(resp.ok, true);
  });

  it("validates error response", () => {
    const resp: WorkerResponse = { id: 1, ok: false, error: "init failed" };
    assert.equal(resp.ok, false);
  });
});
