...
`;
    ...
    for (let i = 1; i < warnings.length; i++) {
      assert.ok(
        warnings[i].start >= warnings[i - 1].start,
        `warning ${i} should come after warning ${i - 1} by position`,
      );
    }
  });
...
describe("UNSUPPORTED_CAPABILITIES", () => {
  ...
  it("each capability has a name and reason", () => {
    for (const cap of UNSUPPORTED_CAPABILITIES) {
      ...
      assert.ok(
        cap.reason.includes("browser") || cap.reason.includes("sandbox"),
        `${cap.id} reason should mention browser/sandbox`,
      );
    }
  });
});
...
describe("getCapabilityInfo", () => {
  ...
  it("returns undefined for unknown ID", () => {
    const info = getCapabilityInfo("nonexistent" as CapabilityId);
