 ...
 */
const PLAIN_KINDS = new Set<string>(["Newline", "Eof", "Error"]);
 ...
 */
const PREFIX_MAP: ReadonlyArray<[string, HighlightCategory]> = [
  // Numeric literals
  ["IntLit", "number"],
  ["FloatLit", "number"],
  ["TypedIntLit", "number"],
 ...
 */
export function classifyTokenKind(kind: string): HighlightCategory {
 ...
 */
export function categoryClass(category: HighlightCategory): string {
 ...
 * ```
 */
export function highlightTokens(source: string, tokens: Token[]): string {
  const parts: string[] = [];
  let pos = 0;
  ...
  for (const token of tokens) {
    ...
    if (category === "plain") {
      ...
      parts.push(
        `<span class="${categoryClass(category)}">${escapeHtml(token.text)}</span>`,
      );
    }
