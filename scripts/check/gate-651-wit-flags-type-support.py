#!/usr/bin/env python3
"""Close gate for issue #651 — WIT flags type support."""
...
def _static_evidence() -> tuple[int, str]:
    required = [
        REPO_ROOT / "src/compiler/component/wit_parse_flags.ark",
        REPO_ROOT / "src/compiler/component/emit_flags.ark",
        REPO_ROOT / "src/compiler/component/export_shapes_flags_scan.ark",
        REPO_ROOT / "src/compiler/component/self_check_issue651.ark",
        REPO_ROOT / "tests/fixtures/component/export_flags_roundtrip.ark",
        REPO_ROOT / "tests/fixtures/component/export_flags_roundtrip.expected.wit",
        REPO_ROOT / "tests/fixtures/component/import_flags_type.ark",
        REPO_ROOT / "tests/fixtures/component/import_flags_type.wit",
    ]
    ...
    if "E0090: unsupported WIT flags type" in wit_text:
        return 1, "wit_text.ark still rejects WIT flags with E0090"
    type_defs = (REPO_ROOT / "src/compiler/component/wit_type_defs.ark").read_text(encoding="utf-8")
    if "wit_export_flags_type_name" not in type_defs or "emit_wit_flags_type_def" not in type_defs:
        return 1, "wit_type_defs.ark missing flags emission helpers"
    manifest = (REPO_ROOT / "tests/fixtures/manifest.txt").read_text(encoding="utf-8")
    for entry in (
        "component-compile:component/export_flags_roundtrip.ark",
        "component-compile:component/import_flags_type.ark",
    ):
        if entry not in manifest:
            return 1, f"manifest missing {entry}"
    if "compile-error:component/import_flags_type.ark" in manifest:
        return 1, "manifest still lists import_flags_type as compile-error"
    return 0, ""
...
def _overlay_evidence() -> tuple[int, str]:
    ...
    emit_flags = overlay / "src/compiler/component_emit_flags.ark"
    wit_type_defs = overlay / "src/compiler/component_wit_type_defs.ark"
    if not emit_flags.is_file() or not wit_type_defs.is_file():
        return 1, "flat overlay missing flags component modules"
    if "emit_flags_roundtrip_component" not in emit_flags.read_text(encoding="utf-8"):
        return 1, "overlay component_emit_flags.ark missing roundtrip emitter"
    if "wit_export_flags_type_name" not in wit_type_defs.read_text(encoding="utf-8"):
        return 1, "overlay component_wit_type_defs.ark missing flags type name helper"
    return 0, ""
...
def _emit_wit_matches_golden(fixture_rel: str, golden_rel: str) -> tuple[int, str]:
    ...
    out_rel = ".build/gate-651-export-flags-roundtrip.wit"
    ...
    golden = golden + "\n" if golden else ""
    if emitted != golden:
        return 1, "export_flags_roundtrip --emit wit diverges from golden"
    return 0, ""
...
def main() -> int:
    ...
    out_dir = Path(tempfile.mkdtemp(prefix="gate-651-", dir=REPO_ROOT / ".build"))
    try:
        export_out = out_dir / "export_flags_roundtrip.component.wasm"
        import_out = out_dir / "import_flags_type.component.wasm"
        for label, fixture, wit, out in (
            (
                "export flags roundtrip",
                "component/export_flags_roundtrip.ark",
                None,
                export_out,
            ),
            (
                "import flags type",
                "component/import_flags_type.ark",
                "component/import_flags_type.wit",
                import_out,
            ),
        ):
            rc, msg = _compile_component(fixture, wit, out)
            if rc == 2:
                print(f"gate-651-wit-flags-type-support: SKIP (dynamic compile: {msg})")
                break
            if rc != 0:
                ...
                if static_rc == 0 and overlay_rc == 0:
                    print(
                        "gate-651-wit-flags-type-support: PASS (static+overlay; dynamic compile skipped)"
                    )
                    return 0
                ...
                if vrc == 2:
                    print(f"gate-651-wit-flags-type-support: SKIP (wasm-tools: {vmsg})")
                elif vrc != 0:
                    static_rc, _ = _static_evidence()
                    overlay_rc, _ = _overlay_evidence()
                    if bootstrap_validate_skip_allowed(vmsg, static_rc, overlay_rc):
                        print(
                            "gate-651-wit-flags-type-support: PASS "
        ...
        if not failures:
            rc, msg = _emit_wit_matches_golden(
                "component/export_flags_roundtrip.ark",
                "component/export_flags_roundtrip.expected.wit",
            )
            if rc == 2:
                print(f"gate-651-wit-flags-type-support: note ({msg})")
            elif rc != 0:
                static_rc, _ = _static_evidence()
                overlay_rc, _ = _overlay_evidence()
                if static_rc == 0 and overlay_rc == 0 and not failures:
                    print(
                        "gate-651-wit-flags-type-support: PASS (static+overlay; dynamic WIT emit skipped)"
                    )
                    return 0
    ...
    if failures:
        print("gate-651-wit-flags-type-support: FAIL", file=sys.stderr)
        for line in failures:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print("gate-651-wit-flags-type-support: PASS")
    return 0
