...
def main() -> int:
    ...
    if compiler is None:
        print("error: no selfhost compiler wasm available for LSP gate", file=sys.stderr)
        return 1
    ...
    for script in scripts:
        ...
        if r_stdio.returncode != 0:
            print(f"FAIL {rel}: lsp stdio exit {r_stdio.returncode}", file=sys.stderr)
            print(r_stdio.stderr[:500].decode("utf-8", errors="replace"),
                  file=sys.stderr)
            failures += 1
            continue
