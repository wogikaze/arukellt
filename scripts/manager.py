...
def main() -> int:
    ...
    if args.domain in ("fmt", "lint"):
        root = _repo_root()
        # Large generated sources (e.g. core_op_registry_generated.ark) exceed
        # the 512 MiB linear memory of the unpatched s2/s3 wasm under parallel
        # fmt/lint. Build the heap-patched s2-runtime (4 GiB) once before
        # formatting/linting so each wasmtime invocation has enough heap.
        if not dry_run:
            try:
                from lib.build_paths import selfhost_dir
                s2 = selfhost_dir(root) / "arukellt-s2.wasm"
