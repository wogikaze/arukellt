...
def extract_doc_entry(path: Path, base_dir: Path) -> DocEntry:
    ...
    in_code = False
    # Preamble metadata common in research/ADR-style docs — not useful as Summary.
    meta_prefixes = (
        "ステータス:",
        "ステータス：",
        "status:",
    ...
    def is_meta(text: str) -> bool:
        lower_plain = text.lower().replace("*", "")
        return any(lower_plain.startswith(p) for p in meta_prefixes)
    ...
    def is_structure(text: str) -> bool:
        return (
            ...
            or text.startswith("<!--")
            or text == "---"
            or text.startswith("> This file is generated by")
        )
    ...
    for line in lines:
        ...
        if stripped.startswith(">"):
            ...
            if not stripped:
                continue
        if is_structure(stripped) or is_meta(stripped):
            if summary_parts:
                break
            continue
        summary_parts.append(stripped)
        joined = " ".join(summary_parts)
        # Soft-wrapped JP/EN paragraphs: keep joining until we have a full clause.
        if len(joined) >= 80 and joined[-1] in "。．.!?)）":
            break
        if len(joined) >= 160:
            break

    summary = escape_table(" ".join(summary_parts)) if summary_parts else "See the document for details."
    return DocEntry(
...
def collect_markdown_entries(section_dir: Path) -> list[DocEntry]:
    ...
    for path in sorted(section_dir.rglob("*.md")):
        if path.name == "README.md":
            continue
        # Probe/result dumps and other nested harness artifacts are not section memos.
        rel = path.relative_to(section_dir).as_posix()
        if rel.startswith("wat-probes/"):
            continue
