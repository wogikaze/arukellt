...
def render_adr_readme(section: dict, entries: list[DocEntry], snapshot_lines: list[str]) -> str:
    ...
    lines.extend(
        [
            "",
            "Status lifecycle: [ADR-0000-process.md](ADR-0000-process.md)",
            "(`PROPOSED` → `ACCEPTED` → `SUPERSEDED`; auxiliary: `REJECTED` / `DEFERRED`).",
            "",
            "Registry gate: `python3 scripts/check/check-adrs.py`.",
        ]
    )
