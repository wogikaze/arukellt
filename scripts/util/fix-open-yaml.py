#!/usr/bin/env python3
"""Fix YAML frontmatter errors in open files by moving invalid content to body."""
...
def fix_dir(issue_dir: Path, dir_name: str):
    ...
    for issue_file in md_files:
        ...
        try:
            ...
            try:
                ...
                fixed += 1
                print(f"Fixed {dir_name}: {issue_file.name}")
            except Exception as e:
                errors += 1
                print(f"Failed to fix {dir_name}/{issue_file.name}: {e}", file=sys.stderr)
    
    print(f"{dir_name} Summary:")
    print(f"  Fixed: {fixed}")
...
def main():
    """Fix all files with YAML errors."""
    total_fixed = 0
    total_skipped = 0
    total_errors = 0
    
    fixed, skipped, errors = fix_dir(OPEN_DIR, "open")
    total_fixed += fixed
    total_skipped += skipped
    total_errors += errors
    
    fixed, skipped, errors = fix_dir(DONE_DIR, "done")
    total_fixed += fixed
    total_skipped += skipped
    total_errors += errors
