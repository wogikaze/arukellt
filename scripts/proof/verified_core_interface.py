...
def interface_payload(function: dict[str, Any]) -> dict[str, Any]:
    """Return the exact callee interface whose contracts are consumed by VC generation."""
    return {
    ...
    for function in document["functions"]:
        for block in function["body"]["blocks"]:
            for instruction in block["instructions"]:
                ...
                if callee_id not in digests:
                    raise ValueError(f"unknown callee id {callee_id}")
                instruction["callee_interface_sha256"] = digests[callee_id]
    return document


def validate_call_interface_binding(
    ...
    if actual is None and call_interfaces_may_be_unbound():
        return
    if not isinstance(actual, str) or len(actual) != 64 or any(ch not in "0123456789abcdef" for ch in actual):
        raise ValueError(f"{path}.callee_interface_sha256: expected lowercase sha256")
    expected = interface_sha256(callee)
    if actual != expected:
        raise ValueError(f"{path}.callee_interface_sha256: callee interface digest mismatch")


__all__ = [
    "allow_unbound_call_interfaces",
    "bind_call_interfaces",
    "call_interfaces_may_be_unbound",
    "interface_payload",
    "interface_sha256",
    "validate_call_interface_binding",
]
