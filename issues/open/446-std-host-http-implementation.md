...
**Reopen reason**: Every artifact this issue's acceptance depended on has been
deleted by the selfhost-first migration (#559 / #583, ADR-029). The
`std::host::http` product claim is no longer backed by any executable code in
the repository, yet the capability manifest still advertises it as available.

**Repo evidence (canonical truth = current files):**

- `crates/` directory is **absent** — the entire Rust workspace (including
  `crates/arukellt/src/runtime.rs`, which held `register_http_host_fns`,
  `http_get_impl`, `http_request_impl`) was deleted. `arukellt_host` /
  `register_http_host_fns` now appear **only** in `std/`, `docs/`, and
  `std/manifest.toml` — never in executable source.
- The sole execution path is `wasmtime run <selfhost.wasm>`
  (`scripts/run/arukellt-selfhost.sh`); there is no `arukellt_host` Wasmtime
  linker providing HTTP host functions anymore.
- The selfhost compiler's host-call dispatch `src/compiler/wasm/call_host_io.ark`
  handles only `env`, `fs` (exists/read/write), `process::exit`, and `stdio`.
  It has **no** `__intrinsic_http_get` / `__intrinsic_http_request` dispatch, so
  a program that calls `std::host::http::get` cannot lower the intrinsic on the
  only compile path.
- `scripts/run/verify-harness.sh` (cited by acceptance "13/13 pass") is **absent**.
- `std/manifest.toml` still declares `std::host::http` with
  `availability = { t1 = true, t3 = true, note = "... arukellt_host::http_get ..." }`
  — a user-visible capability over-claim against a deleted runtime.

**Violated acceptance** (rolled back below): all six completion conditions —
T1/T3 `http::get` runnable via Wasmtime linker, fixtures CI-pass,
`capability-surface.md` enforcement edits, target-matrix accuracy, and
`verify-harness.sh` 13/13 — none are satisfiable with the current repo.

**Evidence files**: `src/compiler/wasm/call_host_io.ark`,
`std/manifest.toml` (`std::host::http` block), `std/host/http.ark`,
`scripts/run/arukellt-selfhost.sh`, `tests/fixtures/host/http/`,
`docs/stdlib/604-contract-honesty-gap-ledger.md` (absence of `crates/`).

**Follow-up split**: capability-surface / manifest honesty correction tracked
as new issue #633 (docs-ahead-of-reality); native WASI P2 HTTP remains #077.
...
> Audit note (2026-06-11, wave 3): rolled back to unmet. The Rust `arukellt_host`
> runtime that satisfied these conditions was deleted; the selfhost compiler has
> no HTTP intrinsic dispatch and `verify-harness.sh` no longer exists. Re-verify
> against the current `wasmtime run <selfhost.wasm>` path before re-closing.

- [ ] T1 で `http::get(url)` が実行可能（エラーケース fixture が現行 selfhost path で CI pass）
- [ ] T3 で `http::get(url)` が実行可能（エラーケース fixture が現行 selfhost path で CI pass）
- [ ] `__intrinsic_http_get` / `__intrinsic_http_request` を selfhost compiler (`src/compiler/wasm/`) が dispatch する
- [ ] エラーマッピング仕様が `docs/capability-surface.md` に明文化されている
- [ ] `std/manifest.toml` の `std::host::http` availability が実態と一致している（過大主張を解消）
- [ ] エラーケース fixture が現行 verify path で pass
- [ ] `docs/current-state.md` Known Limitations explicitly states HTTPS is not supported (docs-to-issues audit 2026-06-12)
- [ ] `docs/capability-surface.md` HTTP section documents TCP HTTP/1.1 only (no HTTPS claim)
