...
- [ ] T1 で `http::get(url)` が Wasmtime linker 経由で実行可能（エラーケース fixture が CI pass）
- [ ] T3 で `http::get(url)` が Wasmtime linker 経由で実行可能（エラーケース fixture が CI pass）
- [ ] エラーマッピング仕様が `docs/capability-surface.md` に明文化されている
- [ ] `docs/capability-surface.md` の "compile-time blocked" 記述が sockets 専用に修正されている
- [ ] Target Compatibility Matrix の http 行が実態と一致している
