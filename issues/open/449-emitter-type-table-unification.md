...
- [ ] T1 / T3 emitter に `TODO(MIR-01)` コメントが 0 件
- [ ] checker 由来のフォールバックパスが T1/T3 emitter から削除されている（または元から存在しないことが確認されている）
- [ ] MIR validation が type_table 整合性チェックを含む
- [ ] `tests/fixtures/regression/type_table/` の fixture が T1/T3 で pass
- [ ] `bash scripts/run/verify-harness.sh` が 13/13 pass（全 592 fixture が pass のまま）
