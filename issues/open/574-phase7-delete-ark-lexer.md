...
`crates/ark-lexer` is targeted for deletion in Phase 7 of #529. This issue performs **only** the deletion of that single crate and the immediate workspace / dependency / CI references to it. No other crate is touched.
...
- [ ] No remaining `cargo run -p ark-lexer`-style invocation anywhere reachable from `scripts/` or `.github/workflows/` (verified by `rg "ark-lexer" scripts/ .github/workflows/`)
- [ ] All consumers of `ark_lexer` symbols outside the crate itself have already been migrated to selfhost (`src/`) or to a remaining crate (verified by `rg "ark_lexer" crates/ src/ scripts/` showing only the crate itself plus explicitly-allowed comments)

## Acceptance

- [ ] `crates/ark-lexer/` directory removed (`[ ! -d crates/ark-lexer ]`)
- [ ] Workspace `Cargo.toml` `members` array no longer lists `crates/ark-lexer`
- [ ] No other crate's `Cargo.toml` lists `ark-lexer` as a `[dependencies]` / `[dev-dependencies]` entry (`grep -RIn "^ark-lexer\b\|\"ark-lexer\"" crates/*/Cargo.toml` empty)
- [ ] `Cargo.lock` regenerated (run `cargo metadata --format-version 1 --offline 2>/dev/null || cargo check --workspace`) and committed without `name = "ark-lexer"`
