...
1. Core Wasm に `wasi: cli/environment@0.2.x` 等を直接 import するセクション生成
...
1. `ark-wasm/src/component/wrap.rs`: P2 ネイティブの場合 `component new` を迂回
1. WIT world 出力を `wasi: cli/command` ベースに変更
...
1. Component Model は `wasi: "cli/run/run` export を期待しているが、T3 emitter は `_start` を生成 (部分的に解決済み)"
2026-04-19 現在の実装状況:
...
## Attempt evidence — 2026-06-14 (lane10-074, still open)

Source-side work landed in-tree (not yet in pinned selfhost wasm):

- `wasm/intrinsics/helpers_io_p2.ark` + `intrinsic_stdio.ark` P2 branch (canonical ABI list write, import 0)
- `wasm/component_p2_emit.ark` appends `p2_run_emit_command_world_sections` (exports `wasi:cli/run@0.2.6`)
- `component/emit.ark` routes non-specialized `--wasi-version p2` output through `wasm::emit_p2_command_component`
- `gate_074` compiles to `.build/close-gate-074/` (avoids `/tmp` write issues)

**Close gate status with current pinned compiler** (`bootstrap/arukellt-selfhost.wasm` / `arukellt-pinned-bootstrap.wasm`):

| Gate item | Status | Evidence |
|-----------|--------|----------|
| P2 compile hello | ✅ | `compile … -o .build/close-gate-074/hello.component.wasm` succeeds |
| wasm-tools validate | ✅ | validate passes on emitted component |
