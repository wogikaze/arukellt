...
**Reopen reason:** README claims five settings control LSP server behaviour; #479 server-side implementation missing.

**Violated acceptance:** README accuracy for settings → LSP behavior mapping

**Evidence files:**
- `extensions/arukellt-all-in-one/README.md`
- `absent LspConfig in src/`

**Follow-up split issue:** see #634 for stdio LSP/DAP transport where applicable

# Extension README: 設定一覧テーブル追加

1. テーブルの各行に: 設定名・型・デフォルト値・説明 の 4 列がある
...
- `extensions/arukellt-all-in-one/README.md` updated with a "Extension Settings" section listing all 5 settings (#477/#478/#479): `arukellt.enableCodeLens`, `arukellt.hoverDetailLevel`, `arukellt.diagnostics.reportLevel`, `arukellt.useSelfHostBackend`, `arukellt.check.onSave`
- Each entry documents: key name, type, default value, and description matching `package.json` declarations exactly
- `grep -E "enableCodeLens|hoverDetailLevel|diagnostics.reportLevel|useSelfHostBackend|check.onSave" README.md` returns 5 hits
- `bash scripts/run/verify-harness.sh --quick`: 19/19 passed
