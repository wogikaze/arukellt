...
- [ ] No remaining `cargo run -p ark-dap`-style invocation anywhere reachable from `scripts/` or `.github/workflows/` (verified by `rg "ark-dap" scripts/ .github/workflows/`)
- [ ] All consumers of `ark_dap` symbols outside the crate itself have already been migrated to selfhost (`src/`) or to a remaining crate (verified by `rg "ark_dap" crates/ src/ scripts/` showing only the crate itself plus explicitly-allowed comments)

## Acceptance

- [ ] `crates/ark-dap/` directory removed (`[ ! -d crates/ark-dap ]`)
- [ ] Workspace `Cargo.toml` `members` array no longer lists `crates/ark-dap`
- [ ] No other crate's `Cargo.toml` lists `ark-dap` as a `[dependencies]` / `[dev-dependencies]` entry (`grep -RIn "^ark-dap\b\|\"ark-dap\"" crates/*/Cargo.toml` empty)
- [ ] `Cargo.lock` regenerated (run `cargo metadata --format-version 1 --offline 2>/dev/null || cargo check --workspace`) and committed without `name = "ark-dap"`
