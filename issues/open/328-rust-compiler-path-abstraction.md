...
- `scripts/verify-harness.sh`: Rust binary path を直接使用
- `.github/workflows/ci.yml`: `cargo build -p arukellt` + `./target/release/arukellt` を直接参照
- `scripts/run-benchmarks.sh`: Rust binary 前提
- perf baseline: Rust binary の compile time / output size を記録

## Acceptance

- [ ] `verify-harness.sh` が `$ARUKELLT_BIN` 環境変数を受け付け、デフォルトは Rust binary
- [ ] CI で selfhost / Rust binary を切り替え可能な matrix job が定義される
- [ ] perf baseline script が compiler binary を引数で受け付ける
- [ ] 切り替え時に fixture 結果の差分が明示的に出力される
