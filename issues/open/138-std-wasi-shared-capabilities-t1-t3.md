# `std::wasi` 共通 capability (`cli` / `filesystem` / `clocks` / `random`) を T1/T3 両対応で実装
...
T1/T3 の両方で意味を共有できる host capability を `std::wasi::<capability>` として提供する。
対象は `std::wasi::cli`, `std::wasi::filesystem`, `std::wasi::clocks`, `std::wasi::random` とし、
backend 差分は namespace ではなく target-specific lowering で吸収する。

## 受け入れ条件

1. 上記 4 module の public API が `std/manifest.toml` と `std/*.ark` に追加される
...
1. `std::wasi::cli` に args / env / exit 相当の facade を定義する
2. `std::wasi::filesystem` に path / read / write / metadata の最小 surface を定義する
3. `std::wasi::clocks`, `std::wasi::random` を target-specific backend へ接続する
