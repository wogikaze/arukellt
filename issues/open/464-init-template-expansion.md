...
**Reopen reason**: Selfhost `cmd_init` (`src/compiler/main/init.ark`) generates only a single
minimal template. No `--template`, `--list-templates`, or alternate template bodies exist.

**Violated acceptance**: All completion conditions for `cli`, `with-tests`, `wasi-host`
templates and `--list-templates`.

**Evidence**:
- `src/compiler/main/init.ark` — single Hello World scaffold only
- `rg 'template|list-templates' src/compiler/main/` — no init template flags
- Deleted `crates/arukellt/` paths cited in original close evidence
...
- [ ] `arukellt init my-project --template cli` が動き、`arukellt check / run` が通る
- [ ] `arukellt init my-project --template with-tests` が動き、`arukellt test` が通る
- [ ] `arukellt init my-project --template wasi-host` が動き、`arukellt check --target wasm32-wasi-p2` が通る
- [ ] `arukellt init --list-templates` がテンプレート一覧を出力する
- [ ] `cargo test -p arukellt -- init_templates` が全 pass する
- [ ] `docs/current-state.md` に `arukellt init --template` の記載がある
