...
- [ ] CI に `selfhost-parity` ジョブが存在し、Rust 実装と selfhost 実装の両方で同一 fixture を実行して結果を比較する
- [ ] `CLI parity`（同一入力に対して同一 stdout/stderr）が検証されている
- [ ] `diagnostic parity`（エラーメッセージの内容・位置情報）が検証されている
- [ ] parity 乖離が検出された場合に diff が CI ログに出力される

## Scope

- `scripts/check-selfhost-parity.sh`（または同等スクリプト）の実装
