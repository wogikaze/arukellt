...
- [ ] CI に `selfhost-parity` ジョブが存在し、Rust 実装と selfhost 実装の両方で同一 fixture を実行して結果を比較する — **実態**: 専用ジョブなし。`selfhost-bootstrap` 内の 1 ステップで `--fixture` のみ。
- [ ] `CLI parity`（同一入力に対して同一 stdout/stderr）が検証されている — **実態**: CI 未実行（スクリプトの `--cli` はローカル向け）。
- [ ] `diagnostic parity`（エラーメッセージの内容・位置情報）が検証されている — **実態**: CI 未実行（`--diag` はローカル向け）。スクリプトは部分一致 + selfhost 未実装時は skip。
- [ ] parity 乖離が検出された場合に diff が CI ログに出力される — **実態**: 要約行と artifact に出力; 完全 diff やジョブ失敗ではない。
