...
- [ ] CI runs both Rust and selfhost compilers for all major fixtures
- [ ] Compare results (exit code, stdout/stderr) for every fixture run
- [ ] Any mismatch is investigated and documented before merge
- [ ] Rust compiler is treated as read-only fallback (no new Rust features)
- [ ] All selfhost bug fixes are also applied to Rust sources where applicable
- [ ] `python scripts/manager.py selfhost fixpoint` rc=0 on every CI run (no regression)
- [ ] `python scripts/manager.py selfhost fixture-parity` FAIL=0 on every CI run
- [ ] `python scripts/manager.py selfhost diag-parity` FAIL=0 on every CI run

## Exit Conditions

The dual-run period ends when ALL of the following hold for 2+ consecutive weeks:

- [ ] Fixpoint stable: `python scripts/manager.py selfhost fixpoint` rc=0 with no regressions
- [ ] Fixture parity stable: `python scripts/manager.py selfhost fixture-parity` FAIL=0 with no regressions
- [ ] No critical diagnostic differences: `python scripts/manager.py selfhost diag-parity` FAIL=0
- [ ] No selfhost regressions in recent changes (verified in CI)
- [ ] `python scripts/manager.py verify quick` 22/22 checks pass
