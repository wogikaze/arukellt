...
- [ ] module 解決規則（ファイルパス・名前空間・再エクスポート）が仕様として文書化されている
- [ ] package 境界の定義（何が 1 package を構成するか）が明確になっている
- [ ] workspace 発見アルゴリズムが仕様として記述されている
- [ ] dependency 解決の優先順位（local path / workspace / registry の順）が定義されている
