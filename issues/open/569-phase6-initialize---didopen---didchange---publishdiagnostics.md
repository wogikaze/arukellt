...
- [ ] `src/ide/lsp.ark` exists with a stdio-loop entry point
- [ ] Implements: `initialize`, `textDocument/didOpen`, `textDocument/didChange`, `textDocument/publishDiagnostics`
- [ ] `didChange` triggers `analyze` from #568 and emits `publishDiagnostics`
- [ ] At least one e2e fixture under `tests/fixtures/ide/lsp_*.ark` (or runner test) drives the four handlers and asserts the response shape
