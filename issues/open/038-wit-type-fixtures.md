...
- [ ] `docs/platform/abi-reference.md` の WIT 型対応表を 16 種全て埋める
- [ ] 非対応型 (flags → E0090, resource → phase 2 pending) の診断コードを記載
...
- [ ] 11 種の新 fixture が `tests/fixtures/manifest.txt` に登録されている
- [ ] `cargo test -p arukellt --test harness` が全 fixture で pass する
  (string/list/resource は `compile-error:` で E009x 診断が正しく出ることを確認)
- [ ] `docs/platform/abi-reference.md` の対応表が 16 種全て記載されている
- [ ] `scripts/verify-harness.sh` の全ゲートが通る
