...
- [ ] collections / seq family に対する高密度 fixture が追加される。
- [ ] 空入力・重複・順序保持などのエッジケースがテストで固定される。
- [ ] canonical API を優先した recipe または examples が追加される。
- [ ] 回帰時に壊れた family を特定できる test grouping が用意される。
