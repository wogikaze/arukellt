...
### Step 3 完了: 文字列フォールバックの集約 (2026-07-16)

トレーサ削除後に残存していた散在する文字列ベースの GC 型推論フォールバックを
集約し、削減した。

#### アプローチ

`SelfEmitCtx_wasm_ref_type_idx_for_type_name` に `gc_layout_table_ref_offset_for_type_name`
の集約フォールバックを追加。これにより、type_id lookup 失敗時に GcLayoutTable モジュール内の
一元化された name→offset マッピングが使用され、呼び出し元側の文字列プレフィックスマッチングが不要になった。

#### 削減した文字列フォールバック

1. **`code_ref_locals_typename.ark::infer_gc_type_from_type_name`**:
   - hashmap/vec/string/option/result/enum の文字列プレフィックスマッチングを削除
   - 残存: struct のみ（`struct:TypeName:byte_size` 形式の名前抽出が必要なため）

2. **`code_ref_locals_infer_callee.ark::infer_call_result_gc_type_from_fn`**:
   - vec/string/hashmap/option/result/enum の文字列フォールバックを削除
   - GcLayoutTable lookup の集約フォールバックで同等の機能を提供

3. **`gc_layout_table.ark::gc_layout_table_ref_offset_for_type_name`**:
   - `string`（小文字）マッチングを追加（旧コードの `eq(type_name, "string")` に対応）

#### 残存する文字列マッチング（今後の課題）

- `should_gc_ref_cast_to_dest` の type_name 文字列マッチング（削除試行 → runtime smoke gate リグレッション → 復元）
- `infer_string_callee_gc_type` / `infer_hashmap_callee_gc_type` / `infer_builtin_callee_gc_type` のハードコード callee 名
- `mir_fn_returns_option_by_name` のハードコード関数名リスト
- `is_gc_container_type_name` / `is_option_type_name` の type_name プレフィックスマッチング

これらは `should_gc_ref_cast_to_dest` が「cast が必要か」の判定に type_name 分類を使用しており、
GcLayoutTable lookup への置換とは異なるセマンティクスを持つため、別途対応が必要。

#### 検証結果

- T3: **389 pass, 32 validate-fail, 1 compile-fail, 22 skip** (total 444) — 変更なし
- verify quick: 3 失敗（全て baseline と同じ既存失敗）
