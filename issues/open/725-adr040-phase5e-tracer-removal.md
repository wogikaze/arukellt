...
トレーサ `infer_ref_local_gc_type_depth` は最終フォールバックとして残存しており、
約100テストケースで機能している。完全削除を試みたところ、286 pass / 136 validate-fail
に悪化した（388 pass / 33 validate-fail から -102 pass / +103 validate-fail）。
...
### Step 2 試行: トレーサ削除 — GcLayoutTable カバレッジ不足で保留

トレーサを `0 - 1` 返却に置換し、T3 を実行したところ:
- **309 pass / 113 validate-fail**（388 pass / 33 validate-fail から -79 pass / +80 validate-fail）

post_pass_type_propagate により issue 予測の 286/136 よりは改善したが、
依然として重大な回帰が発生。トレーサを復元して Step 1 のみ完了とした。

**残存するトレーサの機能（post-pass では代替不可）**:

1. **ハードコードされた callee 名 → GC type index マッピング**:
   - `infer_string_callee_gc_type`: "String_new", "concat", "to_string" 等 → string_type
   - `infer_hashmap_callee_gc_type`: "__hm_si_new" 等 → hashmap_str_i32_type
   - `infer_builtin_callee_gc_type`: "Vec_new_String" → vec_string_type, "split" → vec_string_type 等
   - これらは type_name → GcLayoutTable lookup では解決できない
   （type_name が設定されていても GcLayoutTable にエントリがない場合がある）

2. **Producer inspection による GC type index 直接返却**:
   - トレーサは `infer_ref_local_from_producer` で GC type index を直接返す
   - post-pass は type_name/variant_slot を設定するのみで、
     GcLayoutTable での解決が必要
   - GcLayoutTable にエントリがない型（hashmap, vec の特定型等）は解決不可

**Step 2 完了に必要な追加作業**:
- [ ] GcLayoutTable のカバレッジ拡大（hashmap, vec 特定型等）
- [ ] `infer_builtin_callee_gc_type` のハードコード名を SignatureRegistry に移行
- [ ] `infer_gc_type_from_type_name` の文字列マッチングを GcLayoutTable に統合
- [ ] トレーサ削除後に T3 pass 数が 388 以上を維持
...
- [ ] T3 pass 数が 388 以上を維持
- [ ] T3 validate-fail が 33 以下を維持
