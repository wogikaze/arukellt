...
- [ ] `std/manifest.toml` の全公開 API に stability フィールドが付与されている
- [ ] 各 API の互換ポリシー（stable / provisional / experimental）が明記されている
- [ ] deprecated API には非推奨導線（代替 API・移行方法）が記述されている
- [ ] stdlib リファレンスドキュメントにラベルが反映されている
...
- `python3 scripts/generate-docs.py` で生成される docs への反映
