...
> - 現在: **392 pass / 29 validate-fail**（本表・件数は不変）
...
> - `stdlib_trait/ord_sort_by.ark` は empty-stack 解消後、`expected (ref null $type), found i32`
>   （ref-vs-i32）へ移行。pass 件数は増えていない
...
| エラー型 | 件数 |
|----------|------|
| ref-vs-ref (expected (ref null $type), found (ref null $type)) | 17 |
| ref-vs-i32 (expected (ref null $type), found i32) | 7 |
...
- **ref-vs-ref (17)**: from_trait/from_auto_convert, generics/two_params,
  generics_v1/nested_generic_call, generics_v1/trait_dispatch_stdlib,
  integration/word_counter, iterator/custom_iterator,
  stdlib_hashmap/hashmap_typed_remove_extend, stdlib_string/string_split,
  stdlib_trait/debug_trait, stdlib_trait/buf_read, stdlib_trait/display_trait_vec,
...
- **ref-vs-i32 (7)**: stdlib_io/fs_read_error, stdlib_io/fs_read_write,
  stdlib_trait/debug_vec, stdlib_trait/ord_sort_by,
  stdlib_toml/toml_full_inline_dotted, stdlib_toml/toml_full_table_header,
  stdlib_json/json_perf_decode
...
- [ ] タスク2: ref-vs-i32（7件）解決 — `try`/`match` payload 抽出の local VT 修正
...
| エラー型 | 件数 | エラーメッセージ |
|----------|------|-----------------|
| **ref-vs-ref** | 17 | `expected (ref null $type), found (ref null $type)` |
| **ref-vs-i32** | 7 | `expected (ref null $type), found i32` |
...
## タスク2: ref-vs-i32（7件）— ref が i32 に退化
...
- `stdlib_trait/ord_sort_by.ark` — empty-stack 解消後に本クラスへ移行（2026-07-09）

### 根本原因

`try` / `match` の payload 抽出で、`res_ptr` / `scrut_idx` などの local が GC target でも `VT_GC_REF` ではなく `VT_I32` として宣言されている。
`ord_sort_by` は `call_indirect` 周辺で ref 期待位置に i32 が残る別経路（タスク2 で追跡）。
...
| 優先度 | タスク | 件数 | 効果 |
|--------|--------|------|------|
| **高** | タスク1: ref-vs-ref | 17 | 17件解決で 29→12 |
| **高** | タスク2: ref-vs-i32 | 7 | 7件解決で 12→5 |
...
タスク1 + タスク2 で24件（83%）が解決し、29→5 に減少する見込み。
