...
| **i32-vs-ref** | 3 | `expected i32, found (ref null $type)` |
...
## タスク3: i32-vs-ref（3件残）— i32 のべき local が ref に
...
- `stdlib_trait/iterator_adapters.ark`（`collect_map_filter_i32` call_indirect 型不一致）
...
- `generics_v1/nested_generic_call.ark` — nested `Vec<Vec<i32>>` の push ソース解決 +
  mono `count__Vec_i32_` 解決（2026-07-10 継続改善）
...
mono 実体化後も i32/ref が混線する。`iterator_adapters` は fn 引数の
`call_indirect` 型インデックス不一致が残る。

### 修正方針

- `fn_index_method_return_type` / `entry_methods_mono` で mono suffix から concrete return を復元（一部完了）
- nested Vec mono / store-target 伝播は完了
- 残りは call_indirect 型表と trait method return パス
...
- `src/compiler/mir/lower/call_mono.ark` / `call_mono_by_type.ark`
- `src/compiler/wasm/intrinsic_vec_type*.ark`
- `src/compiler/wasm/sections_types_sigs_mono.ark`
- `src/compiler/wasm/call_indirect.ark`（iterator_adapters）
...
| 優先度 | タスク | 件数 | 効果 |
|--------|--------|------|------|
| **高** | タスク1: ref-vs-ref | 16 | 16件解決で 19→3 |
| **済** | タスク2: ref-vs-i32 | 0 | 解消済み（393/28） |
| **中** | タスク3: i32-vs-ref | 3 | 3件解決で 19→16（nested_generic_call 済） |
| **済** | タスク4: i32-vs-f64 | 0 | 解消済み |
| **済** | タスク5: empty-stack | 0 | store policy で解消 |
| **済** | タスク6: array-subtype | 0 | `connect_read_write` PASS |

タスク1 で16件、残タスク3 で3件。現行 T3: **402 pass / 19 validate-fail**。

## Notes

### 関連コミット

- （本セッション）nested Vec mono / Range GC ABI / fnref field layout（402/19）
