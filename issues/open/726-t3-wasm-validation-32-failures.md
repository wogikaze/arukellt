...
- [ ] タスク1: ref-vs-ref（22件）解決 — MIR local 型一貫性（emitter 局所パッチは非推奨）
...
| エラー型 | 件数 | エラーメッセージ |
|----------|------|-----------------|
| **ref-vs-ref** | 22 | `expected (ref null $type), found (ref null $type)` |
...
| 優先度 | タスク | 件数 | 効果 |
|--------|--------|------|------|
| **高** | タスク1: ref-vs-ref | 22 | 22件解決で 28→6 |
