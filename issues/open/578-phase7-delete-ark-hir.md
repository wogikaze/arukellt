...
**Implementation target**: Per #529 Phase 7, this issue removes exactly one Rust crate (`crates/ark-hir`). No Ark product code is added or changed; this is retirement work scoped to a single crate.

## Summary

`crates/ark-hir` is targeted for deletion in Phase 7 of #529. This issue performs **only** the deletion of that single crate and the immediate workspace / dependency / CI references to it. No other crate is touched.
...
- [ ] No remaining `cargo run -p ark-hir`-style invocation anywhere reachable from `scripts/` or `.github/workflows/` (verified by `rg "ark-hir" scripts/ .github/workflows/`)
- [ ] All consumers of `ark_hir` symbols outside the crate itself have already been migrated to selfhost (`src/`) or to a remaining crate (verified by `rg "ark_hir" crates/ src/ scripts/` showing only the crate itself plus explicitly-allowed comments)

## Acceptance

- [ ] `crates/ark-hir/` directory removed (`[ ! -d crates/ark-hir ]`)
- [ ] Workspace `Cargo.toml` `members` array no longer lists `crates/ark-hir`
- [ ] No other crate's `Cargo.toml` lists `ark-hir` as a `[dependencies]` / `[dev-dependencies]` entry (`grep -RIn "^ark-hir\b\|\"ark-hir\"" crates/*/Cargo.toml` empty)
- [ ] `Cargo.lock` regenerated (run `cargo metadata --format-version 1 --offline 2>/dev/null || cargo check --workspace`) and committed without `name = "ark-hir"`
- [ ] No source / script / docs reference: `rg -l "\bark_hir\b\|\bark-hir\b" crates/ scripts/ src/ docs/ .github/` returns only entries explicitly enumerated in the close note (e.g. archived ADRs)
...
rg -l "\bark_hir\b" crates/ scripts/ src/ docs/ .github/
```
...
1. [ ] Directory truly absent: `test ! -d crates/ark-hir` exit 0
2. [ ] No workspace member ref: `grep -F "crates/ark-hir" Cargo.toml` empty
3. [ ] No reverse dep ref: `grep -RIn "\bark-hir\b" crates/*/Cargo.toml` empty
4. [ ] No Rust source ref: `rg -l "\bark_hir\b" crates/ src/` empty
5. [ ] No script / CI ref: `rg -l "\bark-hir\b" scripts/ .github/workflows/` empty
6. [ ] No docs ref: `rg -l "\bark_hir\b\|\bark-hir\b" docs/` returns only paths listed in the close note (archived ADRs allowed if explicitly enumerated)
...
- `crates/ark-hir/` (deletion)
- `Cargo.toml` (workspace `members`)
- `Cargo.lock` (regeneration)

## Allowed adjacent paths

- `Cargo.toml` of OTHER crates: **only** to remove a `[dependencies]` / `[dev-dependencies]` entry on `ark-hir`
...
- Suggested message: `chore(crates): remove crates/ark-hir per #529 Phase 7 (closes #578)`
