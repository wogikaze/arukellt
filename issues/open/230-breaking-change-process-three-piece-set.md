...
- [ ] 破壊的変更の定義が文書化されている（何が「破壊的」とみなされるか）
- [ ] 破壊的変更を導入するための必須手順（issue 起票 → changelog → migration guide）が ADR として定められている
- [ ] 既存の変更履歴から破壊的変更に相当するものが遡及して changelog に記録されている
- [ ] CHANGELOG.md または同等のファイルが存在し、バージョン別に管理されている
