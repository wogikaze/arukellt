...
**Reopen reason:** LspConfig struct and handler behavior cited in `crates/ark-lsp` — crate deleted; no selfhost equivalent.

**Violated acceptance:** All five initializationOptions affect LSP handler behavior with tests

**Evidence files:**
- `absent crates/ark-lsp/`
- `rg LspConfig under src/ (no matches)`
...
1. `initializationOptions` に `enableCodeLens: false` を渡して codeLens リクエストを送ると
2. `initializationOptions` に `hoverDetailLevel: "minimal"` を渡して hover リクエストを送ると
3. `initializationOptions` に `diagnostics.reportLevel: "errors"` を渡すと
