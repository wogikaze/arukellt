...
- [ ] Browser UI exposes format action that calls engine format API and updates editor content
- [ ] Browser UI exposes tokenize action or uses tokenize for syntax highlighting beyond parse-only path
- [ ] docs/playground/README.md format/tokenize rows updated to repo-proved (regenerate via generate-docs.py if needed)
- [ ] docs/playground/README.md typecheck tracking row updated (#472 is done; remove stale issues/open/472 reference)
- [ ] playground npm build succeeds and pages workflow deploy path remains green
