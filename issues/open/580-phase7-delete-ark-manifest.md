...
Implementation target: "Per #529 Phase 7, this issue removes exactly one Rust crate (`crates/ark-manifest`). No Ark product code is added or changed; this is retirement work scoped to a single crate."
...
- Suggested message: "`chore(crates): remove crates/ark-manifest per #529 Phase 7 (closes #580)`"
commit: <hash>
fixpoint: rc=0 → rc=0
...
`crates/ark-manifest` is targeted for deletion in Phase 7 of #529. This issue performs **only** the deletion of that single crate and the immediate workspace / dependency / CI references to it. No other crate is touched.
