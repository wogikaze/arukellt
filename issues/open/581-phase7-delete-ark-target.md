...
`crates/ark-target` is targeted for deletion in Phase 7 of #529. This issue performs **only** the deletion of that single crate and the immediate workspace / dependency / CI references to it. No other crate is touched.
...
- [ ] No remaining `cargo run -p ark-target`-style invocation anywhere reachable from `scripts/` or `.github/workflows/` (verified by `rg "ark-target" scripts/ .github/workflows/`)
- [ ] All consumers of `ark_target` symbols outside the crate itself have already been migrated to selfhost (`src/`) or to a remaining crate (verified by `rg "ark_target" crates/ src/ scripts/` showing only the crate itself plus explicitly-allowed comments)

## Acceptance

- [ ] `crates/ark-target/` directory removed (`[ ! -d crates/ark-target ]`)
- [ ] Workspace `Cargo.toml` `members` array no longer lists `crates/ark-target`
- [ ] No other crate's `Cargo.toml` lists `ark-target` as a `[dependencies]` / `[dev-dependencies]` entry (`grep -RIn "^ark-target\b\|\"ark-target\"" crates/*/Cargo.toml` empty)
- [ ] `Cargo.lock` regenerated (run `cargo metadata --format-version 1 --offline 2>/dev/null || cargo check --workspace`) and committed without `name = "ark-target"`
