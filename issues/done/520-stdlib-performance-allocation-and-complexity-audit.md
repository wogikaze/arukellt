...
- [x] family ごとの perf footgun inventory が作成される
- [x] `concat` 連鎖, linear scan, needless allocation, repeated parse の 4 類型で分類される
- [x] `text::builder`, buffered I/O, pre-sized vec, better search strategy など推奨置換パターンが決まる
- [x] benchmark へ繋げるべき hotspot が特定される

## Close note — 2026-05-14

This issue is closed as an audit/inventory slice. The documented inventory now covers
the targeted `std::json`, `std::csv`, `std::io`, and `std::bytes` families, classifies
each hotspot by the requested perf-footgun axes, records replacement patterns, and
lists benchmark follow-ups by family. No source-level implementation change is included
because the issue scope is explicitly audit inventory rather than remediation of the
listed hotspots.
