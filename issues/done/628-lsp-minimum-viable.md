...
**Reopen reason:** LSP MVP handlers exist via script replay, but user-visible stdio transport entrypoint is not wired (`cmd_lsp` requires input file).

**Violated acceptance:** stdio transport for JSON-RPC usable by VS Code LanguageClient
...
## Audit resolution — 2026-06-14 (Lane 17 hygiene)

**Classification:** `truly-done`

**Repo proof:** Handler slices #569 and #570 remain done; stdio transport gap from the 2026-06-12 reopen is closed by #634 (`src/compiler/main/editor.ark` — `cmd_lsp` reads framed JSON-RPC from stdin when no script argument). `python3 scripts/check/check-lsp-lifecycle.py`: PASS, 21 scripts. `python scripts/manager.py verify quick`: PASS, 163/163.

**Action:** Moved to `issues/done/`. Further LSP feature work stays in #333–#342 / #450–#454; epic gaps under #183.
