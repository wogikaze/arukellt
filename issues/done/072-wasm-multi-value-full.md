...
- `cargo test -p ark-wasm tuple_ifexpr_uses_multivalue_block_and_single_struct_new -- --nocapture`
	passed.
- `bash scripts/run/verify-harness.sh --quick` passed.
- `cargo build --workspace` is blocked in this environment by `llvm-sys` because LLVM 18 is not
	installed for `ark-llvm`.
- `cargo build --workspace --exclude ark-llvm` passed, which matches the repository's normal local
	verification guidance for this workspace.
