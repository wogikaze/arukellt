...
The remaining implementation gap for this issue was the v4-compatible
deprecation path for legacy local-file imports. The compiler now keeps
`import <single_identifier>` source-compatible while emitting `W0101` at parse
time with a fix-it to `use <single_identifier>`.

Implemented:

- `DiagnosticCode::W0101` (`warning`, `parse`) for deprecated legacy import syntax
- parser warning and fix-it for `import name` and `import name as alias`
- lint registry entry for `W0101` under the migration category
- docs updates in `docs/compiler/error-codes.md`,
  `docs/compiler/diagnostics.md`, `docs/current-state.md`, and ADR-009

Verification:

- `cargo test -p ark-diagnostics -p ark-parser` passed
- `python3 scripts/check/check-docs-consistency.py` passed

Layer C source syntax and WIT pipeline wiring remain tracked by #124 and related
