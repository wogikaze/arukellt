...
**Repo proof:** All slice children are in `issues/done/` (#565 lexer recovery, #566 parser partial AST with `NK_ERROR`/`NK_MISSING` in `src/compiler/parser/kinds.ark`, #567 resolver/typechecker incremental diagnostics). The 2026-06-12 reopen reason (#566 error-node contract) is resolved — see `issues/done/566-phase6-partial-ast-recovery.md` and fixture `tests/fixtures/selfhost/parser_recovery_decls.ark`.

**Action:** Moved to `issues/done/`. Aggregate rollup only; incremental reparse remains #099.
