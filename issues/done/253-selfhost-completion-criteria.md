...
* [x] `scripts/run/verify-bootstrap.sh` が skip 前提の scaffold ではなく達成判定の本体になっている（267完了）
* [x] Stage1 fixture parity・CLI parity・diagnostic parity が CI で継続検証されている（268完了）
* [x] Rust 実装と selfhost 実装の dual period を終わらせる条件が定義されている（ADR-027）
