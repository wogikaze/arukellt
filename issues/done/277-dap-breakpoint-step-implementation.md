...
- [ ] `setBreakpoints` でソース行にブレークポイントを設定でき、ヒットした時に停止できる
- [ ] `configurationDone` リクエストを処理できる
- [ ] `continue` でブレークポイントから実行を再開できる
- [ ] `next`（ステップオーバー）でソース行単位のステップ実行ができる
- [ ] `disconnect` でデバッグセッションを正常終了できる
