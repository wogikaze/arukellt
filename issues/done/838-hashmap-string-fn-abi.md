...
Blocks v{N}: none
Priority: 2
Source: #845 Non-goal — HashMap&lt;String, fn&gt; after HashMap&lt;i32, fn&gt;
---
...
#845 で `HashMap&lt;i32, fn&gt;`（`__hm_if_*`）は閉じた。
`HashMap&lt;String, fn&gt;` は未実装で、String key（`__hm_ss_*`）と funcref value（`__hm_if_*`）の組み合わせが無い。

## Strategy

- **分類**: 実装拡張（#845 / monomorphic HashMap 特化の穴埋め）。新 ADR 不要。
- **ABI**: GC-only。keys=`A_ref0`(String)、vals=`A_fnref`、flags=`A_i32`。`fnref_array` は #845 と共用。
