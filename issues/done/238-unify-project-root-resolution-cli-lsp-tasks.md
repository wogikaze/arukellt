...
- [x] project root の解決が CLI・LSP・Tasks provider で同じ結果を返す
- [x] `[targets]` の選択が CLI コンパイルと LSP の解析で一致する
- [x] `[scripts]` が CLI・Tasks provider・サイドバーで同じリストを返す
- [x] manifest の変更後に LSP が自動再読み込みする
