...
- [x] `ark.toml` のスキーマ仕様が文書化されている（必須フィールド・オプショナルフィールド・デフォルト値）
- [x] CLI の全コマンドが `ark.toml` を参照して動作する
- [x] `ark.toml` がない場合の単一ファイルモードとの違いが明確に定義されている
- [x] スキーマ違反時に actionable なエラーメッセージが出る
