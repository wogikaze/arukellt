...
Closed: 2026-06-10
Closed note: |
  All implementations confirmed working:
  - Arena, SlotMap, Interner in `std/collections/compiler.ark`
  - Rope in `std/text/rope.ark`
  - 6 test fixtures registered in manifest.txt and passing (317/317, 0 failures)
  - Fixtures: arena_basic, slotmap_basic, slotmap_remove, interner_basic, rope_basic, rope_edit
  - Generic `<T>` variants deferred to future issue (monomorphization blocker #312)
...
**Blocker status: STALE - UNBLOCKED.** The original assumption that Arena/SlotMap/Interner/Rope required special emitter GC intrinsics was incorrect. All four data structures are already implemented as pure Ark code using existing emitter intrinsics:

- **Arena** (`std/collections/compiler.ark`): `Vec<i32>` backing + monotonic ID counter
- **SlotMap** (`std/collections/compiler.ark`): generation-indexed array using `Vec<i32>`
- **Interner** (`std/collections/compiler.ark`): `Vec<String>` backing with linear search for dedup
- **Rope** (`std/text/rope.ark`): flat `String` backing (O(n) ops; balanced tree deferred)

**Evidence:**
- `slotmap_basic.ark` compiles and runs correctly
- `interner_basic.ark` compiles and runs correctly
- `rope_basic.ark` compiles and runs correctly
- All fixtures registered as `run:` in manifest.txt
- Dependencies 039 and 041 are both DONE (verified)

**Action:** Reclassify from `blocked-by-upstream` to `implementation-ready`. Remaining work: generify Arena/SlotMap from monomorphic i32 to generic `<T>`, upgrade Interner from linear search to HashMap-backed lookup, upgrade Rope from flat String to balanced tree, expand fixture coverage to 6+.
