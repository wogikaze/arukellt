...
- [x] T1 で `http::get(url)` が実行可能（エラーケース fixture が現行 selfhost path で CI pass）
- [x] T3 で `http::get(url)` が実行可能（エラーケース fixture が現行 selfhost path で CI pass）
- [x] `__intrinsic_http_get` / `__intrinsic_http_request` を selfhost compiler (`src/compiler/wasm/`) が dispatch する
- [x] エラーマッピング仕様が `docs/capability-surface.md` に明文化されている
- [x] `std/manifest.toml` の `std::host::http` availability が実態と一致している（過大主張を解消）
- [x] エラーケース fixture が現行 verify path で pass
- [x] `docs/current-state.md` Known Limitations explicitly states HTTPS is not supported (docs-to-issues audit 2026-06-12)
- [x] `docs/capability-surface.md` HTTP section documents TCP HTTP/1.1 only (no HTTPS claim)
