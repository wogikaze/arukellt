...
- [ ] No remaining `cargo run -p ark-driver`-style invocation anywhere reachable from `scripts/` or `.github/workflows/` (verified by `rg "ark-driver" scripts/ .github/workflows/`)
- [ ] All consumers of `ark_driver` symbols outside the crate itself have already been migrated to selfhost (`src/`) or to a remaining crate (verified by `rg "ark_driver" crates/ src/ scripts/` showing only the crate itself plus explicitly-allowed comments)

## Acceptance

- [ ] `crates/ark-driver/` directory removed (`[ ! -d crates/ark-driver ]`)
- [ ] Workspace `Cargo.toml` `members` array no longer lists `crates/ark-driver`
- [ ] No other crate's `Cargo.toml` lists `ark-driver` as a `[dependencies]` / `[dev-dependencies]` entry (`grep -RIn "^ark-driver\b\|\"ark-driver\"" crates/*/Cargo.toml` empty)
- [ ] `Cargo.lock` regenerated (run `cargo metadata --format-version 1 --offline 2>/dev/null || cargo check --workspace`) and committed without `name = "ark-driver"`
- [ ] No source / script / docs reference: `rg -l "\bark_driver\b\|\bark-driver\b" crates/ scripts/ src/ docs/ .github/` returns only entries explicitly enumerated in the close note (e.g. archived ADRs)
...
1. [ ] Directory truly absent: `test ! -d crates/ark-driver` exit 0
2. [ ] No workspace member ref: `grep -F "crates/ark-driver" Cargo.toml` empty
3. [ ] No reverse dep ref: `grep -RIn "\bark-driver\b" crates/*/Cargo.toml` empty
4. [ ] No Rust source ref: `rg -l "\bark_driver\b" crates/ src/` empty
5. [ ] No script / CI ref: `rg -l "\bark-driver\b" scripts/ .github/workflows/` empty
6. [ ] No docs ref: `rg -l "\bark_driver\b\|\bark-driver\b" docs/` returns only paths listed in the close note (archived ADRs allowed if explicitly enumerated)
