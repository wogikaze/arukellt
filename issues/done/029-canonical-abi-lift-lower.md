...
- [x] A `canonical_abi` module exists at `crates/ark-wasm/src/component/canonical_abi.rs`
      containing `lower_*` and `lift_*` functions for each bridged type.
- [x] **String lower**: `(ref (array (mut i8)))` → copy bytes to linear memory → return `(i32, i32)`.
- [x] **String lift**: `(i32 ptr, i32 len)` from linear memory → `array.new_data` or byte-copy
      into GC array → return `(ref (array (mut i8)))`.
- [x] **List lower**: `(ref $vec_T)` → extract array ref + len → copy elements to linear memory
      → return `(i32, i32)`. Element lowering recurses for nested types.
- [x] **List lift**: `(i32, i32)` → allocate GC array + struct → copy elements from linear memory
      → return `(ref $vec_T)`.
- [x] **Record lower/lift**: flatten struct fields to/from canonical scalar sequence.
- [x] **Variant lower/lift**: discriminant + payload to/from canonical flat representation.
      Covers `option<T>` and `result<T, E>` as variant specializations.
- [x] **Scalar pass-through**: `i32`, `i64`, `f32`, `f64`, `bool`, `char` require no conversion
      (validated by tests).
- [x] Linear memory allocation strategy for canonical ABI buffers is defined: reuse the existing
      ...
      component call).
- [x] Unit tests for each lift/lower pair with round-trip verification (lower then lift = identity).
- [x] The `GcTypeRegistry` in `t3_wasm_gc.rs` is extended if needed to generate helper functions
