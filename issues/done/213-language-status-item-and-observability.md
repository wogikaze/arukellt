...
- [x] language status item が LSP サーバーの現在状態（indexing / ready / error 等）を表示する
- [x] output channel が用途別に分類されている（LSP / compiler / test / task）
- [x] verbose log toggle と server trace level 切り替えが設定またはコマンドから行える
