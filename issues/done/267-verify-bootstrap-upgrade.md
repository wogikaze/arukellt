...
`scripts/verify-bootstrap.sh` は Stage 0 で `main.wasm` が生成されない場合に Stage 1/2 を skip する構造になっており、fixpoint の継続検証が担保されていない。このスクリプトを skip 前提の scaffold から達成判定の本体へ昇格させる。
...
- `scripts/verify-bootstrap.sh` の skip 条件を除去
- 各 stage の失敗ログを詳細化
