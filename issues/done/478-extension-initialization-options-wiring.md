...
- `extension.js` initializationOptions block (lines 192–200): all 5 keys present
  (`enableCodeLens`, `hoverDetailLevel`, `diagnosticsReportLevel`, `useSelfHostBackend`, `checkOnSave`)
- `workspace/didChangeConfiguration` handler at line 245 re-sends all 5 settings on config change
- `bash scripts/run/verify-harness.sh --quick`: PASS (19/19 checks)
- No changes to `crates/ark-lsp/`
