...
Implemented `--compare` and `--selfhost` flags in `scripts/run-benchmarks.sh`:
...
Verification: `bash scripts/run-benchmarks.sh --quick --compare` — runs successfully, shows fib/binary_tree/string_concat at 1.1-1.7x compile overhead with 0.48-0.54x binary size.
