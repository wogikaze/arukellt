...
impl Parser<'_> {
    ...
    pub(crate) fn parse_item(&mut self, docs: Vec<String>) -> Option<Item> {
        // Accept multiple consecutive `pub` keywords (selfhost source uses
        // redundant `pub pub pub ... pub fn` patterns for overlay promotion).
        let is_pub = self.eat(&TokenKind::Pub);
        while is_pub && *self.peek() == TokenKind::Pub {
            self.advance(); // skip redundant `pub`
        }
        match self.peek() {
