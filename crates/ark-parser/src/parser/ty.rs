    ...
    pub(crate) fn parse_type_expr(&mut self) -> TypeExpr {
        ...
        match self.peek().clone() {
            ...
            TokenKind::Ident(name) => {
                self.advance();
                // Check for qualified type: mod.Type or mod::Type or a::b::c::Type
                ...
                if *self.peek() == TokenKind::ColonColon {
                    // Multi-level path: a::b::c::Type — collect all segments
                    let mut parts = vec![name];
                    while *self.peek() == TokenKind::ColonColon {
                        self.advance();
                        parts.push(self.expect_ident());
                    }
                    // The last part is the type name, the rest is the module path
                    let type_name = parts.pop().unwrap();
                    let module = parts.join("::");
                    // Check for generics after qualified type
