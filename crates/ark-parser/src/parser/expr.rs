    ...
    fn parse_primary(&mut self) -> Expr {
        ...
        match self.peek().clone() {
            ...
            TokenKind::Ident(name) => {
                self.advance();
                // Qualified: Name::Variant or a::b::c or module.name
                if *self.peek() == TokenKind::ColonColon {
                    // Multi-level path: a::b::c — collect all segments
                    ...
                    if *self.peek() == TokenKind::LBrace {
                        let qualified = format!("{}::{}", module, variant);
            ...
            TokenKind::RParen | TokenKind::Comma => {
                // Selfhost-compatible: silently skip stray `)` or `,` in expression
                // position (selfhost source has redundant closing parens).
                let sp = self.span();
                self.advance();
                Expr::IntLit {
                    value: 0,
