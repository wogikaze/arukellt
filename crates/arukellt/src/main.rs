...
fn compile_wit(path: &PathBuf) -> Result<String, String> {
    let source =
        std::fs::read_to_string(path).map_err(|e| format!("error: {}: {}", path.display(), e))?;
    ...
    let world = ark_wasm::component::mir_to_wit_world(&mir, world_name)
        .map_err(|e| format!("WIT generation error: {}", e))?;
    ark_wasm::component::generate_wit(&world).map_err(|e| format!("WIT generation error: {}", e))
}
