...
/// Handle `arukellt run <file.ark>` — compile via selfhost, then execute the
/// resulting wasm with wasmtime, streaming stdout/stderr and exit code.
fn cmd_run(run_args: &[String]) -> ExitCode {
    ...
    // Step 1 — compile the input file via the selfhost compiler.
    // We create a temp output path next to the input with a .wasm extension.
    ...
    // Build args: `compile <input> -o <output> --emit wasm`
    ...
    // Pass through any additional args after the input file (e.g. --target)
    ...
    let compile_status = match compile_cmd.status() {
        ...
        Err(e) => {
            eprintln!(
                "arukellt: failed to invoke `wasmtime` for compilation: {e}\n\
                 Install wasmtime ≥ 30."
            );
            return ExitCode::from(127);
        }
    ...
    if !compile_status.success() {
        // Compilation failed — forward the exit code.
        return ExitCode::from(compile_status.code().unwrap_or(1) as u8);
    }

    // Step 2 — run the compiled wasm with wasmtime.
    let mut run_cmd = Command::new("wasmtime");
    run_cmd.current_dir("/");
    run_cmd.arg("run").arg("--dir=/::/");
    run_cmd.arg(format!("--dir={}", root.display()));
    if let Ok(cwd) = env::current_dir()
        && cwd.is_dir()
        && cwd != root
        && cwd != Path::new("/")
    {
        run_cmd.arg(format!("--dir={}", cwd.display()));
    }
    ...
    // Clean up the temporary wasm output.
    let _ = std::fs::remove_file(&out_path);
...
fn main() -> ExitCode {
    ...
    // Intercept `arukellt run <file>` — special two-step: compile then execute.
