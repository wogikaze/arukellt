...
impl EmitCtx {
    ...
    pub(super) fn call_fn(&self, f: &mut Function, canonical: u32) {
        ...
        if idx == u32::MAX {
            // Function not available (e.g., arukellt_host imports skipped) — push 0 as fallback
    ...
    /// Resolve a function by name and preferred argument count.
    /// When multiple functions share the same name (e.g., from different modules),
    /// prefer the one whose parameter count matches the call's argument count.
    pub(super) fn resolve_fn_with_argc(&self, name: &str, argc: usize) -> Option<u32> {
        let matches: Vec<_> = self
            ...
            .filter(|(_, n)| {
                n.as_str() == name
                    ...
                    || name.rsplit("::").next() == Some(n.as_str())
            })
            .collect();
        if matches.is_empty() {
            return None;
        ...
        if matches.len() == 1 {
            let idx = self.fn_map[FN_USER_BASE as usize + matches[0].0];
            return if idx == u32::MAX { None } else { Some(idx) };
        }
        // Multiple matches: prefer one with matching param count
        for (i, _) in &matches {
            let fn_idx = self.fn_map[FN_USER_BASE as usize + i];
            if fn_idx == u32::MAX {
                continue;
            }
            // Check param count from fn_param_types
        ...
        // Fall back to first valid match
        for (i, _) in &matches {
            let fn_idx = self.fn_map[FN_USER_BASE as usize + i];
            if fn_idx != u32::MAX {
                return Some(fn_idx);
    ...
    pub(super) fn build_user_fn(&mut self, func: &MirFunction) -> Function {
        ...
        // Run this BEFORE building locals list so we can override local types
        ...
        // Scan statements to identify additional typed locals from operand context
        for block in &func.blocks {
            self.scan_string_locals(&block.stmts);
        }

        // Build locals list with proper types, using scanned type info for overrides
        let mut locals = Vec::new();
        if num_locals > num_params {
            for local in &func.locals[num_params as usize..] {
                let valtype = if self.i64_locals.contains(&local.id.0) {
                    ValType::I64
                } else if self.f64_locals.contains(&local.id.0) {
                    ValType::F64
                } else if self.f32_locals.contains(&local.id.0) {
                    ValType::F32
                } else {
                    Self::type_to_valtype(&local.ty)
                };
                locals.push((1, valtype));
            }
        }
        ...
        // Debug: dump MIR for specific functions
        if std::env::var("ARK_DUMP_MIR").is_ok()
            && (func.name == "cmd_dap" || func.name.contains("run_dap") || func.name == "read_to_string"
                || func.name == "node_int_val" || func.name == "mir_int_literal_needs_i64"
                || func.name == "finish_decimal_number" || func.name == "toml_parse_value"
        ...
        if std::env::var("ARK_DUMP_FNMAP").is_ok() && func.name == self.fn_names.first().map(|s| s.as_str()).unwrap_or("") {
            for (i, n) in self.fn_names.iter().enumerate() {
                let idx = self.fn_map[FN_USER_BASE as usize + i];
                eprintln!("FNMAP[{}] = idx {} = {}", i, idx, n);
            }
        }
            ...
            match &block.terminator {
                Terminator::Return(Some(op)) => {
                        ...
                        && !matches!(func.return_ty, ark_typecheck::types::Type::Unit)
                    {
                        ...
                        // Type coercion for return: widen i32 -> i64 if function returns i64
                        let ret_is_i64 = matches!(
                            func.return_ty,
                            ark_typecheck::types::Type::I64 | ark_typecheck::types::Type::U64
                        );
                        let ret_is_f64 = matches!(func.return_ty, ark_typecheck::types::Type::F64);
                        let op_is_i64 = self.is_i64_operand(op);
                        let op_is_f64 = self.is_f64_operand(op);
                        if ret_is_i64 && !op_is_i64 {
                            f.instruction(&Instruction::I64ExtendI32S);
                        } else if ret_is_f64 && !op_is_f64 && !op_is_i64 {
                            f.instruction(&Instruction::F64ConvertI32S);
                        } else if ret_is_f64 && op_is_i64 {
                            f.instruction(&Instruction::F64ConvertI64S);
                        } else if !ret_is_i64 && !ret_is_f64 && op_is_i64 {
                            f.instruction(&Instruction::I32WrapI64);
                        } else if !ret_is_i64 && !ret_is_f64 && op_is_f64 {
                ...
                Terminator::TailCall { func: callee, args } => {
                    ...
                    self.emit_operand(&mut f, &op);
                    // Type coercion for tail call: widen i32 -> i64 if function returns i64
                    let ret_is_i64 = matches!(
                        func.return_ty,
                        ark_typecheck::types::Type::I64 | ark_typecheck::types::Type::U64
                    );
        ...
        for stmt in stmts {
            match stmt {
                MirStmt::Assign(Place::Local(id), Rvalue::Use(op)) => {
                    ...
                    // Track f64/i64 locals from operand types
                    if self.is_f64_operand(op) {
                        self.f64_locals.insert(id.0);
                    }
                    if self.is_i64_operand(op) {
                        self.i64_locals.insert(id.0);
                    }
                    // Track bool locals from function calls returning bool
                    if let Operand::Call(name, _) = op {
                        let lookup = name.rsplit("::").next().unwrap_or(name.as_str());
                        let normalized = normalize_intrinsic_name(lookup);
                        if let Some(rt) = self.fn_return_types.get(normalized)
                            .or_else(|| self.fn_return_types.get(lookup))
                            .or_else(|| self.fn_return_types.get(name.as_str()))
                        {
                            if matches!(rt, ark_typecheck::types::Type::Bool) {
                                self.bool_locals.insert(id.0);
                            }
                        }
                    }
                    // Track f64/i64 from EnumPayload by enum payload info
                    if let Operand::EnumPayload { enum_name, variant_name, index, .. } = op {
                        let (_, is_f64, is_i64) = self.enum_payload_info(enum_name, variant_name, *index as usize);
                        if is_f64 {
                            self.f64_locals.insert(id.0);
                        }
                        if is_i64 {
