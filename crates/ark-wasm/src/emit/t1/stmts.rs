    ...
    pub(super) fn emit_stmt(&mut self, f: &mut Function, stmt: &MirStmt) {
        match stmt {
            MirStmt::Assign(Place::Local(id), Rvalue::Use(op)) => {
                ...
                // Type coercion: widen i32 -> i64 when local is i64 but operand is i32
                let local_is_i64 = self.i64_locals.contains(&id.0);
                let local_is_f64 = self.f64_locals.contains(&id.0);
                let op_is_i64 = self.is_i64_operand(op);
                let op_is_f64 = self.is_f64_operand(op);
                if local_is_i64 && !op_is_i64 {
                    f.instruction(&Instruction::I64ExtendI32S);
                } else if local_is_f64 && !op_is_f64 && !op_is_i64 {
                    f.instruction(&Instruction::F64ConvertI32S);
                } else if local_is_f64 && op_is_i64 {
                    f.instruction(&Instruction::F64ConvertI64S);
                } else if !local_is_i64 && !local_is_f64 && op_is_i64 {
                    // Truncate i64 -> i32 when local is i32 but operand is i64
                    f.instruction(&Instruction::I32WrapI64);
                } else if !local_is_i64 && !local_is_f64 && op_is_f64 {
                    // Truncate f64 -> i32 when local is i32 but operand is f64
            ...
            MirStmt::Assign(Place::Field(base, field_name), Rvalue::Use(op)) => {
                ...
                self.emit_operand(f, op);
                // Type coercion for field store
                let op_is_f64 = self.is_f64_operand(op);
                let op_is_i64 = self.is_i64_operand(op);
                if is_f64 && !op_is_f64 && !op_is_i64 {
                    f.instruction(&Instruction::F64ConvertI32S);
                } else if is_f64 && op_is_i64 {
                    f.instruction(&Instruction::F64ConvertI64S);
                } else if is_i64 && !op_is_i64 {
                    f.instruction(&Instruction::I64ExtendI32S);
                } else if !is_f64 && !is_i64 && op_is_i64 {
                    f.instruction(&Instruction::I32WrapI64);
                } else if !is_f64 && !is_i64 && op_is_f64 {
                    f.instruction(&Instruction::I32TruncF64S);
                }
            ...
            MirStmt::CallBuiltin {
                ...
                // Type coercion for call result assignment
                let local_is_i64 = self.i64_locals.contains(&id.0);
                let local_is_f64 = self.f64_locals.contains(&id.0);
                let op_is_i64 = self.is_i64_operand(&call_op);
                let op_is_f64 = self.is_f64_operand(&call_op);
                if local_is_i64 && !op_is_i64 {
                    f.instruction(&Instruction::I64ExtendI32S);
                } else if local_is_f64 && !op_is_f64 && !op_is_i64 {
                    f.instruction(&Instruction::F64ConvertI32S);
            ...
            MirStmt::CallBuiltin { name, args, .. } => {
                ...
                match name {
                    ...
                    "process_abort" => {
                        // process::abort() -> ! — SIGABRT convention: proc_exit(134)
                    ...
                    other => {
                        ...
                        // Push defaults for missing params (module merge name collisions)
                            ...
                            .or_else(|| self.resolve_fn(lookup_name))
                        {
                            ...
                            // Unknown function: drop emitted args
                            for _ in args.iter() {
                                f.instruction(&Instruction::Drop);
                            }
                        }
