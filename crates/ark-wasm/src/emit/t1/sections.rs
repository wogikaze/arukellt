...
impl EmitCtx {
    pub(super) fn emit_module(&mut self, mir: &MirModule) -> Vec<u8> {
        ...
        // Skip HOST_ORDER — arukellt_host imports are not emitted for selfhost compatibility
        ...
        // arukellt_host imports (http_get, http_request) — skipped for selfhost compatibility
        // The bootstrap wasm does not include these imports; they are only needed for
        // network-enabled targets. Skipping them avoids instantiation failures.
