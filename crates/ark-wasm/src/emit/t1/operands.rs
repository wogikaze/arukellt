...
impl EmitCtx {
        ...
        match op {
            ...
            Operand::Call(name, args) => {
                ...
                match name {
                    ...
                    "http_get" => {
                        ...
                        // If host import is not available, return Result::Err("http not available")
                        if self.fn_map[FN_HTTP_GET as usize] == u32::MAX {
                            // Args are not yet emitted by this point — just push a dummy result
                            f.instruction(&Instruction::I32Const(0)); // dummy enum ptr
                            return;
                        }
                    ...
                    "http_request" => {
                        ...
                        // If host import is not available, return dummy result
                        if self.fn_map[FN_HTTP_REQUEST as usize] == u32::MAX {
                            // Args are not yet emitted by this point — just push a dummy result
                            f.instruction(&Instruction::I32Const(0)); // dummy enum ptr
                            return;
                        }
                    ...
                    "clone" => {
                        // clone: dispatch based on argument type
                        // For Vec<String>, clone the Vec struct (12 bytes: len, cap, data_ptr)
                        // For String, deep copy the string data
                        if let Some(a) = args.first() {
                            if self.is_vec_string_operand(a) {
                                let ma = MemArg { offset: 0, align: 2, memory_index: 0 };
                                // Save src ptr to SCRATCH+8
                                f.instruction(&Instruction::I32Const((SCRATCH + 8) as i32));
                                self.emit_operand(f, a);
                                f.instruction(&Instruction::I32Store(ma));
                                // Copy 3 fields (len, cap, data_ptr) from src to new heap
                                for (dst_off, src_off) in [(0, 0), (4, 4), (8, 8)] {
                                    f.instruction(&Instruction::GlobalGet(0));
                                    f.instruction(&Instruction::I32Const(dst_off));
                                    f.instruction(&Instruction::I32Add);
                                    f.instruction(&Instruction::I32Const((SCRATCH + 8) as i32));
                                    f.instruction(&Instruction::I32Load(ma));
                                    f.instruction(&Instruction::I32Const(src_off));
                                    f.instruction(&Instruction::I32Add);
                                    f.instruction(&Instruction::I32Load(ma));
                                    f.instruction(&Instruction::I32Store(ma));
                                }
                    ...
                    other => {
                        ...
                        // If the function has more params than args provided, push defaults
                        // for the missing params (handles module merge name collisions).
                        if let Some(pts) = &param_types {
                            for i in args.len()..pts.len() {
                                match pts[i] {
                        ...
                        if let Some(idx) = self
                            .resolve_fn_with_argc(other, args.len())
                            .or_else(|| self.resolve_fn_with_argc(lookup_name, args.len()))
                            .or_else(|| self.resolve_fn(other))
                            .or_else(|| self.resolve_fn(lookup_name))
                        {
                            f.instruction(&Instruction::Call(idx));
                        } else {
                            // Unknown function: drop emitted args and push 0 as fallback
                            for _ in &args[..] {
                                f.instruction(&Instruction::Drop);
                            }
                            f.instruction(&Instruction::I32Const(0));
            ...
            Operand::EnumInit {
                ...
                for (i, pval) in payload.iter().enumerate() {
                    ...
                    // Type coercion for enum payload store
                    let pval_is_f64 = self.is_f64_operand(pval);
                    let pval_is_i64 = self.is_i64_operand(pval);
                    if is_f64 && !pval_is_f64 && !pval_is_i64 {
                        f.instruction(&Instruction::F64ConvertI32S);
                    } else if is_f64 && pval_is_i64 {
                        f.instruction(&Instruction::F64ConvertI64S);
                    } else if is_i64 && !pval_is_i64 {
                        f.instruction(&Instruction::I64ExtendI32S);
                    } else if !is_f64 && !is_i64 && pval_is_i64 {
                        f.instruction(&Instruction::I32WrapI64);
                    } else if !is_f64 && !is_i64 && pval_is_f64 {
            ...
            Operand::Unit => {
                // Push 0 as fallback for Unit in value position
                f.instruction(&Instruction::I32Const(0));
            }
            other => {
                eprintln!(
    ...
    pub(super) fn is_f64_operand(&self, op: &Operand) -> bool {
        match op {
            ...
            Operand::FieldAccess { struct_name, field, .. } => {
                let (_, is_f64, _) = self.struct_field_info(struct_name, field);
                is_f64
            }
            Operand::EnumPayload { enum_name, variant_name, index, .. } => {
                let (_, is_f64, _) = self.enum_payload_info(enum_name, variant_name, *index as usize);
                is_f64
            }
            Operand::IfExpr {
                then_result,
    ...
    pub(super) fn is_i64_operand(&self, op: &Operand) -> bool {
        match op {
            ...
            Operand::FieldAccess { struct_name, field, .. } => {
                let (_, _, is_i64) = self.struct_field_info(struct_name, field);
                is_i64
            }
            Operand::EnumPayload { enum_name, variant_name, index, .. } => {
                let (_, _, is_i64) = self.enum_payload_info(enum_name, variant_name, *index as usize);
                is_i64
            }
            Operand::IfExpr {
                then_result,
    ...
    pub(super) fn emit_i64_operand(&mut self, f: &mut Function, op: &Operand) {
        match op {
            ...
            Operand::Call(_, _) => {
                // Emit the call normally, then extend i32 result to i64
