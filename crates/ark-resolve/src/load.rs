...
pub(crate) fn resolve_import_path(
    ...
    if module_name.starts_with("std") {
        ...
        let rel = module_name.replace("::", "/");
        // Use entry_root (the entry point's directory) as the base for local
        // module resolution, matching the selfhost compiler's behavior.
        let local_path = entry_root.join(format!("{}.ark", rel));
        let local_mod = entry_root.join(&rel).join("mod.ark");
    ...
    F: FnMut(&Path, &mut DiagnosticSink) -> Result<ast::Module, String>,
{
    ...
    if !visiting.insert(path.clone()) {
        // Selfhost-compatible: silently skip circular imports instead of
        // emitting an error. The selfhost compiler treats re-visits as
        // already-loaded and continues without error.
        return;
    }
    ...
    match &import.kind {
        ImportKind::DestructureImport { names } => {
            ...
            for name in names {
                ...
                let import_path =
                    resolve_import_path(current_path, &sub_module, std_root, sink, target, entry_root);
        ...
        _ => {
            ...
            let mut import_path =
                resolve_import_path(current_path, &target_module_name, std_root, sink, target, entry_root);
            ...
            {
                let parent_path =
                    resolve_import_path(current_path, parent_module, std_root, sink, target, entry_root);
    ...
    F: FnMut(&Path, &mut DiagnosticSink) -> Result<ast::Module, String>,
{
    ...
    // entry_root: the directory containing the entry point file.
    // All local module imports are resolved relative to this directory,
    // matching the selfhost compiler's behavior.
    let entry_root = entry_path.parent().unwrap_or_else(|| Path::new("."));
