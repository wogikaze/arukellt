...
3. 次の対応で最小十分な検証を選ぶ。
   ...
   | 変更 | 検証 |
   |---|---|
   | 通常のコード、テスト、例、build/check挙動 | `python3 scripts/manager.py verify quick` |
   | fixture、診断、言語意味論 | quick + `python3 scripts/manager.py verify fixtures` |
   | selfhost compiler / bootstrap | quick +該当する `selfhost fixpoint` / `fixture-parity` / `diag-parity` / `parity` |
   | Component emit / WIT / composition | quick + component emit gate または `verify component-interop` |
