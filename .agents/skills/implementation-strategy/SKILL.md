...
6. 実装の読みやすさは `AGENTS.md` の「コード品質規約」に合わせる。過剰な wrapper・小ファイル分割・深い分岐を増やさない。
7. コンパイラ層・API・診断/ICE・決定性は `docs/process/coding-conventions.md` に合わせる。
8. 完了条件を、観測可能な挙動と実行する検証コマンドで定義する。
