...
| `arukellt.target` | `"wasm32"` \| `"wasm32-gc"` \| `"native-cpp"` \| `"native-llvm"` \| `null` | `null` | LSP (`arkTarget`), tasks, commands. The extension presents canonical ADR-007 names; deprecated aliases remain CLI input compatibility only. |
