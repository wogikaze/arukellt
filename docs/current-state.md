...
3. ✅ **CLI integration**: `--wit <path>`, `--emit wit`, `--emit component`, and `--emit all` are wired into the selfhost CLI.
  ...
  Unsupported WIT import shapes such as `resource` declarations, `own<T>` / `borrow<T>`
  resource handles, and `stream<T>` / `future<T>` async resource shapes are rejected
  with `E0402`. WIT `flags` types are supported for import and export round-trip
...
| Tier 3 | resource, stream, future | Not implemented | E0400/E0402 |
