...
| Tier 3 | stream, future | Not implemented | E0402/#474 |
...
- **Bootstrap vs s2 `--emit wit` for library exports (#666)**: the pinned bootstrap selfhost wasm (`bootstrap/arukellt-selfhost.wasm`) uses a memory-bounded component overlay stub and returns empty WIT for library-style `pub fn` exports. Build or point `ARUKELLT_SELFHOST_WASM` at `.build/selfhost/arukellt-s2.wasm` for non-empty library WIT (`add`/`mul`, scalar round-trip goldens). CI gates treat empty WIT as a failure when the active selfhost wasm is s2; pinned bootstrap round-trip tests may still fall back to committed goldens for bindings smoke only.
