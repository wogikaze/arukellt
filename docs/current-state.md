...
- `native-cpp` is a **scaffold / partial / experimental** target with **`run_supported=true`**: public `arukellt run --target native-cpp` is offered on Linux x86-64 with clang 14+ for the supported subset (ADR-050). Guaranteed public corpus: 16/16 (`docs/data/native-cpp-public-coverage-receipt.json`). **Provisional full-tree probe (v1, not a readiness metric):** `fn main(` filter + exit-0 scoring on `tests/fixtures/**` reported compile ~56% / run ~43% with ICE 88 and crash 83 (`docs/data/native-cpp-fixture-coverage-receipt.json`). Those rates are **unclassified**; official readiness metrics come from measure v2 (manifest populations + semantic expectation match) in [native-cpp-general-backend-readiness.md](plans/native-cpp-general-backend-readiness.md). The **internal** C99 selfhost executor lane remains **experimental**: production root clears, arena/GC dual mode, S2/S3 equality+determinism, and strict wall/RSS dual gate without `--allow-high-rss` (CI rejects that flag). Latest strict 3× evidence: warm wall 238–249 s, peak RSS ≤ 2.345 GiB (`docs/data/native-cpp-executor-promotion-receipt.json`). `native-llvm` retains its fixed GNU assembler scaffold. See [ADR-049](adr/ADR-049-native-c99-selfhost-executor.md), [ADR-050](adr/ADR-050-experimental-public-native-c99-run.md), and [RFC-008](rfcs/008-native-cpp-c99-backend-runtime-abi.md).
