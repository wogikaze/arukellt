...
> Implementation-Commit: `222faab5`
> Documentation-Commit: `9d05db21`
> Fixture-Snapshot-Commit: `222faab5`
...
- Accounting note: 804+367+417=1588 outcomes from the 2026-07-14 selfhost fixture-parity run at 222faab5; 2693 is tests/fixtures/manifest.txt registry size. The 1105 remainder were not part of that run (not proof they fail).
...
|----|-------|----------|---------:|----------|-----------------|---------|-------|-------|------------|---------------|-----------|
| `check_fixture_harness` | `full` | `fixture` | 367 | `incident_fixture_parity_367` | Failures in observed harness snapshot. Same incident as selfhost fixture-parity — not double-counted. See project-state.toml for current registry count. | `python3 scripts/manager.py verify fixtures` | compiler/runtime | #807 | `89eb5eb4` | `222faab5` | `fresh` |
| `check_t3_wasm_validate` | `quick` | `verification` | 1 | `incident_t3_wasm_validate` | T3 fixture Wasm validation fails for 213 run fixtures (func N failed to validate) | `python3 scripts/check/check-t3-wasm-validate.py` | Wasm backend | #808 | `fd14539c23288d3ed993c03600aeed36cd478d06` | `222faab5` | `fresh` |
| `check_selfhost_fixpoint` | `full` | `bootstrap` | 1 | `incident_selfhost_fixpoint` | Stage 2 and Stage 3 compiler hashes differ; fixpoint not reached | `python3 scripts/manager.py selfhost fixpoint --build` | selfhost compiler | #813 | `a80b4181` | `222faab5` | `fresh` |
| `check_selfhost_cli_parity` | `full` | `bootstrap` | 2 | `incident_selfhost_cli_parity` | CLI parity drifts for --help and compose --validate | `python3 scripts/manager.py selfhost parity --mode --cli` | selfhost CLI | #811 | `a80b4181` | `222faab5` | `fresh` |
| `check_selfhost_diag_parity` | `full` | `bootstrap` | 3 | `incident_selfhost_diag_parity` | Selfhost diagnostic parity differs from Rust host compiler | `python3 scripts/manager.py selfhost diag-parity` | selfhost diagnostics | #812 | `a80b4181` | `222faab5` | `fresh` |
| `check_wat_roundtrip` | `full` | `target-contract` | 1 | `incident_wat_roundtrip` | The wasm2wat/wat2wasm roundtrip gate fails | `bash scripts/run/wat-roundtrip.sh` | Wasm backend | #809 | `a80b4181` | `222faab5` | `fresh` |
| `check_component_interop_wasmtime` | `full` | `component-interop` | 103 | `incident_component_interop_103` | All wasmtime component-interop cases fail. Dedicated command (not aggregate verify full). | `python3 scripts/manager.py verify component-interop` | component model | #810 | `a80b4181` | `222faab5` | `fresh` |
