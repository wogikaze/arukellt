...
- **Host capability honesty (#633)** — `std::host::http` and `std::host::udp` are not user-reachable on the current selfhost execution path. `std::host::sockets::connect` is T3-reachable via the host-linker (#139). Manifest and `docs/capability-surface.md` cross-link #446/#447/#077. HTTPS is not supported for HTTP.
...
- **Host module target-gating and reachability**: `std::host::http` and `std::host::udp` are not user-reachable on the current selfhost compile path (see `docs/capability-surface.md` and #633). `std::host::sockets::connect` is available on T3 via the host-linker (#139); importing `std::host::sockets` or `std::host::udp` on T1 produces E0500. `std::host::http` is HTTP/1.1 only when implemented; HTTPS is not supported.
