...
| `native-cpp` | scaffold | partial | experimental | No | Recovered C99 emitter/runtime + native-executor lane; S2/S3 equality green; exact GC RSS ~1.5 GiB (under 2.4 GiB); warm wall still ~10–11 min (5 min gate pending) |
...
- `native-cpp` remains a **scaffold target** with `run_supported=false` (not promoted to experimental completion). The C99 emitter/runtime and `python3 scripts/manager.py selfhost native-executor --build` lane are recovered: S3 inherits the comparison S2 build-profile manifest (`wasm32`/`wasi-p1` today), validates, is deterministic, and same-profile S2/S3 byte equality is green; exact non-moving GC brings executor RSS to ~1.5 GiB (under 2.4 GiB), but warm full-S3 wall is still ~10–11 min (5 min gate pending). `native-llvm` retains its fixed GNU assembler scaffold. See [ADR-049](adr/ADR-049-native-c99-selfhost-executor.md) and [RFC-008](rfcs/008-native-cpp-c99-backend-runtime-abi.md).
