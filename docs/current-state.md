...
- Fixture registry: 2706 manifest entries (distinct unit from harness outcomes)
- Not in last harness snapshot: 1118 registry entries (not proof they fail)
- Accounting note: 57+1089+442=1588 outcomes from the 2026-07-15 selfhost fixture-parity run at 982f3102; 2706 is tests/fixtures/manifest.txt registry size. The 1118 remainder were not part of that run (not proof they fail).
...
- `native-*` is **scaffold-only** (#641): compile-only GNU assembler stub via `native::emit_native_scaffold`; `run_supported=false`. [ADR-049](adr/ADR-049-native-c99-selfhost-executor.md) adopts the native-cpp executor design, but the lane and C99 lowering remain unimplemented.
