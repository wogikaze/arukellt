...
| ステップ | ツール | 結果 |
|---------|--------|------|
| jco transpile 出力の Node.js 実行 | node v23.6.0 (V8) | ✅ 動作確認済み |
| Chrome headless での ESM 読み込み | google-chrome | ❌ `file://` プロトコルで ESM import 不可 |
| 直接 WebAssembly.instantiate | `scripts/perf/run-browser-bench.mjs` | ✅ 既存のベンチマークで動作確認済み |

### 詳細

- **Chrome は Wasm GC をネイティブサポート**: V8 は WasmGC に対応済み
- **Node.js (V8) で動作確認済み**: 同じ V8 エンジンのため、jco transpile 出力は
  Chrome でも動作するはずだが、`file://` プロトコルでは ESM の動的 import が
  セキュリティ制限で失敗する。ローカル HTTP サーバーが必要。
- **ベンチマークインフラあり**: `scripts/perf/run-browser-bench.mjs` は
  puppeteer-core 経由で headless Chrome を起動し、直接 `WebAssembly.instantiate()` で
  Wasm モジュールを実行する。WASI shim も内蔵。
...
4. HTTP サーバー経由で配信（`file://` では ESM import 不可）
...
| wasm32-gc → jco → Node.js | jco 1.25.2 + node 23 | ⚠️ | `arguments` パッチ + npm install 必要 |
| wasm32-gc → jco → Chrome | jco 1.25.2 + chrome | ⚠️ | HTTP サーバー必要。V8 で動作確認済み (Node.js 経由) |
