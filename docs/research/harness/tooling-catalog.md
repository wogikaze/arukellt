...
| Root verify command | Arukellt は `scripts/manager.py` を持ち、参照 repo は CI、`just`、`mise` で completion surface を定義。 | 今すぐ採用: 短い root verify script を常に持つ。 |
...
- root completion gate としての `scripts/manager.py`
