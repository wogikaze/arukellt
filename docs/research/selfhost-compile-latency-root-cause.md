...
| After (#829) | **1.8 s** | 660.9 s | 684.9 s | ≈11.5 min |

支配フェーズは **propagate → emit** に移った。`CalleePropCache` は CALL ごとの
O(関数数) 線形探索と callee 全身スキャンを、モジュール先頭の NameIndex +
事前計算 return metadata に置換する。
...
| `wasm emit` | section/function 単位の再構築・clone・名前検索監査（**現行の支配相**） |
