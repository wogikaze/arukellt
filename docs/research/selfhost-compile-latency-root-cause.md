...
支配フェーズは **propagate → emit.code.locals →（残）emit.code.insts** と移った。
...
- Emit local-GC cache (`code_ref_locals_fn_cache.ark`): 関数先頭で
  ...
  local ごとにメモ化する。以前は `emit_local_decls` が local ごとに
  全身再スキャンしていた（verify-prop で `emit.code.locals` ≈ 412 s）。

Post-fix（`runId=post-fix-emit`）のキャッシュ統計:
`functions=8220 struct_new_hits≈1.79M infer_hits≈495k infer_fills≈172k`。
...
| `emit.code.locals` | ~~per-function GC infer cache~~（landed） |
| `wasm emit` / `emit.code.insts` | call_resolve・struct 名線形探索・#826 clone（**現行の支配相** ≈104 s emit） |
