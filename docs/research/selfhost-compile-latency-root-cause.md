...
内訳（`post-fix-struct-idx`）: `emit.code.locals` **88.8 s** / `emit.code.insts` **1.3 s**。

支配フェーズは **propagate → emit.code.insts →（残）emit.code.locals** と移った。
...
残課題: locals の初回 infer fill がなお body スキャンを行う（≈89 s）。
...
| `emit.code.locals` | ~~struct_new / infer memo~~（landed）; **残 ≈89 s** は初回 fill の body scan |
| `emit.code.insts` | ~~struct NameIndex~~（landed, ≈1.3 s）; 次は call_resolve 衝突時フルスキャン |
