# WasmGC Post-MVP 拡張機能 — 設計調査

ステータス: 調査メモ（決定記録ではない）  
由来: 旧 ADR-008 / 旧 ADR-043 本文（2026-04 調査、2026-07 改訂）  
関連決定: [ADR-043](../adr/ADR-043-wasm-gc-post-mvp.md)

---

本ファイルは Wasm GC Post-MVP 提案の調査記録である。言語仕様の前提にはしない。
採択済みの方針は ADR-043 を参照する。
...
本調査は 2026-04 時点で 6 つの "Post-MVP" 提案を調査したが、その後:
...
本調査は各提案の現状・Arukellt としての活用可能性・実装コスト推定を記録し、
将来設計フェーズへの入力とする。実装決定は ADR-043 に置く。
...
| # | 提案 | W3C/CG ステータス (2026-07) | wasmtime 46 | V8 14.6 | 優先度 (将来設計) |
|---|------|-----------------------------|:-----------:|:-------:|-----------------|
...
- スレッド安全性 (shared-everything-threads 提案) は現スコープ外
...
現時点では「調査・設計のみ」とし、ADR-003 の将来検討項目として管理する。
...
| 提案 | Phase | 活用可能性 | 実装コスト | 将来優先度 | 早期適用可能か |
|------|-------|-----------|-----------|-----------|----------------|
