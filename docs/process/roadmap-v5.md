...
テスト: 同一入力に対する Rust 版と Arukellt 版の出力一致を差分検証 (`scripts/compare-outputs.sh`)。
...
8. **ブートストラップ検証スクリプト** (`scripts/verify-bootstrap.sh`)  
...
| `scripts/verify-bootstrap.sh` が exit code 0 を返す | スクリプト実行 |
