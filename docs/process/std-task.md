...
- ~~`crates/ark-mir/src/lower.rs` (removed in #561)~~
...
- [x] `import_basic` が `arukellt run` で動くようにする
- [x] `module_alias` が `arukellt run` で動くようにする
- [x] wrapper 経由 call の Wasm stack balance 崩れを修正する
- [x] user function call / builtin call / intrinsic call の codegen 規則を整理する
- [x] `println`, `print`, `eprintln`, `String_from`, `eq` の wrapper 経路を end-to-end で安定化する
...
- [x] compiler が知る intrinsic 名一覧を固定する
- [x] public wrapper 名一覧を `std/prelude.ark` 側へ寄せる
- [x] resolver の prelude 注入から public wrapper 名を外す
- [x] typechecker が public 名ではなく intrinsic 名を builtin として持つよう整理する
- [x] emitter の special-case 対象を intrinsic 名中心へ揃える
...
- `crates/ark-typecheck/src/checker.rs`
- ~~`crates/ark-mir/src/lower.rs` (removed in #561)~~
- ~~`crates/ark-mir/src/mir.rs` (removed in #561)~~
- `src/compiler/emitter.ark` (selfhost Wasm emitter)
- `std/core/option.ark`
- `std/core/result.ark`
...
- [x] tuple enum payload lowering を実装する
- [x] `Some(x)` / `Ok(x)` / `Err(x)` の end-to-end 動作を安定化する
- [x] payload binding を伴う `match` を runnable にする
- [x] `Option` / `Result` helper の最低限 (`is_some`, `is_none`, `unwrap_or`) を source 化可能にする
- [x] `?` 演算子対応の前提条件を揃える
...
- [x] Vec の runtime representation を固定する
- [x] `Vec_new_*`, `push`, `pop`, `get`, `set`, `len` を runnable にする
- [x] `Vec<String>` と `Vec<i32>` の動作境界を明確にする
- [x] `get_unchecked` の扱いを決める
- [x] `quickstart` の Vec 章が動く状態に近づける
...
- [x] `tests/harness.rs` の compile/run 比較 TODO を実装する
- [x] module import 系 fixture を追加する
- [x] source-backed prelude fixture を追加する
- [ ] stdlib migration の影響を CI で継続確認できるようにする
- [x] `scripts/manager.py` と実テスト内容の意味を一致させる
...
- [x] `docs/process/v0-status.md` を最新 stage に更新する
- [x] `docs/stdlib/README.md` に「Rust builtin から source stdlib へ移行中」の reality を反映する
- [ ] `docs/quickstart.md` のサンプルを runnable 範囲へ寄せる
- [x] `docs/process/v0-scope.md` の ✅ を実装 reality に合わせる
