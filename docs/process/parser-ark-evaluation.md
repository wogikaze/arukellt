...
**前提**: `break` / `continue` は v0 仕様に含まれている。v1 項目ではない。

**設計原則**: v1 の最優先は「高度な抽象化」ではなく「while 手展開を減らす最低限の制御構文追加」。trait や for と違って解決規則を増やさないものを先に入れる。

### P1: `for` ループ（trait 不要の限定版）

**効果**: 推定 -80〜100 行（parser.ark 基準）

Vec に対する正規反復構文。trait ベースの iterator を急ぐと設計が重くなるが、限定版でもかなり効く:
...
解決規則を増やさない。LLM フレンドリも壊さない。設計コストが低く、while 手展開の大半を解消。LLM のバグ率に最も効く改善。
...
concat ネストの完全解消。diagnostics と std 実装の品質に直結。

### P3: trait / iterator

**効果**: 推定 -60〜80 行（`any`, `find`, `map`, `filter` 等の復活）

ここは効くが、設計コストも高い。v0→v1 の橋としては、まず組み込みの反復プロトコルや Vec 専用高階関数で稼いだ方が安全。

### P4: メソッド構文

**効果**: 読みやすさ向上、行数影響小

`push(v, x)` → `v.push(x)` は見た目の問題。本質改善ではないが、Rust/Go/Python 経験者の学習コストを下げる。trait(P3) の後に入れる。
