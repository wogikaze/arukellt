...
|------------|----------|---------|------------|
| **Snapshot** | `tests/snapshots/` | Deterministic compiler output (MIR dumps, diagnostic messages) | Updated by rerunning the harness (no dedicated snapshot update script); committed alongside the code change that caused the diff |
...
4. **Update scripts are the single entry point for baselines** — never
   hand-edit baseline files. Snapshots lack a dedicated update script;
   they are regenerated by rerunning the harness and reviewed via `git diff`.
