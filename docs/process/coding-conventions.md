...
## 意味論負債（semantic debt）の禁止

次を新規に入れない。既存は allowlist を縮める方向だけを許す。

- 複数フィールドを 1 つの整数へ pack して record / tuple 戻りを回避する
- ABI や型の不整合を truncate / widen / stub で黙殺する
- 「とりあえず」「workaround」だけの恒久パッチ（structured TODO と issue なし）

やむを得ない bridge は (1) 追跡 issue、(2) 削除条件、(3)
`docs/data/semantic-debt-allowlist.toml`、(4) structured TODO を揃える。
検査: `python3 scripts/check/check-semantic-debt.py`（`quality quick` に含まれる）。
