> **ARCHIVED REPORT** — Rust-era bootstrap narrative (pre ADR-029). Not current. See `docs/adr/ADR-029-selfhost-native-verification-contract.md` and `docs/compiler/bootstrap.md`.
...
[state/compiler.md — Self-Hosting Bootstrap Status](../../state/compiler.md#self-hosting-bootstrap-status).
...
| CI `selfhost-bootstrap` job | `scripts/run/verify-bootstrap.sh --check` | Runs Stages 0 → 1 → 2; the workflow asserts Stage 0 and Stage 1 report `reached`; Stage 2 fixpoint is reached; `sha256(s2) == sha256(s3)` (see [state/compiler.md](../../state/compiler.md#self-hosting-bootstrap-status)) |
...
- [IR Specification](../../compiler/ir-spec.md)
...
See [state/compiler.md — Self-Hosting Bootstrap Status](../../state/compiler.md#self-hosting-bootstrap-status)
