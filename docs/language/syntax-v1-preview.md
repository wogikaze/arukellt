...
> **✅ v0 で実装済み**: for ループは v0 スコープに含まれている。
...
> **✅ v0 で実装済み**: 文字列補間は v0 スコープに含まれている。
...
> **✅ trait 定義・実装は v1 で実装済み**（静的ディスパッチのみ）。
> Iterator trait ベースの for ループは未実装。
...
> **✅ v1 で実装済み**: `impl Type { fn method(self) { ... } }` と `obj.method(args)` が動作。
> `obj.method(args)` は `Type__method(obj, args)` に脱糖される（静的ディスパッチ）。
...
> **✅ v1 で実装済み**: `impl` メソッド経由で add/sub/mul/div/eq/cmp をオーバーロード可能。
> `a + b`（同じ struct 型）→ `Type__add(a, b)` に変換される。
