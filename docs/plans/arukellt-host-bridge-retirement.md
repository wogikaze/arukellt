...
> **Superseded (2026-09-12):** This bridged plan is historical. ADR-054
> completes the retirement by deleting `host-linker`, the old network API
> surfaces, and compatibility paths; official WASI P2 component packaging is
> now the only supported boundary.

ステータス: **廃止（2026-09-13）** — 旧 bridged close の履歴。ADR-054 により
host-linker、旧 network API、互換経路を削除し、公式 WASI P2 Component packaging を現行境界とした。
...
Child: [`#830`](../../issues/done/830-wasm-heap-grow-patcher-retirement.md)（patcher 退役）
Follow-up: [`#841`](../../issues/done/841-wit-network-real-wasi-abi.md)（real WASI ABI の履歴）
...
| 過去の bridged 実装 | Guest import module 名を WIT 形へ変更した履歴。簡略 guest ABI と host-linker binding は ADR-054 で廃止。 |
| 現行の境界 | 旧 HTTP/TCP/UDP/stream surface と Rust runtime は削除済み。公式 WASI Component と `wasm-tools` による packaging のみを使用する。 |
...
5. `tools/host-linker` が WIT module 名で bind（カスタム `arukellt_host` 登録なし）。この bridged 条件は履歴であり、runtime とともに ADR-054 で削除済み。
...
| 3 import table | done — module 文字列から `arukellt_host` 除去。flag 名 `needs_runtime_host` は overlay 都合で `#841` へ延期 |
