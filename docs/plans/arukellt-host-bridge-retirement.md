...
| 残ギャップ（`#841`） | 真の WASI method / resource ABI、bare `wasmtime run`、`host_http.rs` / `host_sockets.rs` 削除、`needs_network_runtime` リネーム。 |
...
| 3 import table | done — module 文字列から `arukellt_host` 除去。flag 名 `needs_network_runtime` は overlay 都合で `#841` へ延期 |
