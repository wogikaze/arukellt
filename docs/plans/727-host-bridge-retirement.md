...
- 残ギャップ（`#841`）: 真の WASI method / resource ABI、bare `wasmtime run`、`host_http` / `host_sockets` shim 削除、`needs_arukellt_host` リネーム。
...
- `needs_arukellt_host` フラグ改名は overlay 都合で `#841` へ延期。
...
- `needs_arukellt_host` 名の残存は overlay 都合の既知延期。
