# ADR-000: ADR プロセスとステータスライフサイクル

ステータス: **ACCEPTED** — ADR の識別子・状態遷移・後継関係の規則を固定する

決定日: 2026-07-11

---

## 文脈

`docs/adr/` に設計判断・調査・実装計画・進捗メモが混在し、番号重複・未定義ステータス・
未来日付・欠番参照が発生していた。ADR を意思決定履歴の正本として使うには、
プロセス自体を ADR として固定する必要がある。

新規・改訂する ADR の**本文・見出し・メタデータは日本語を正**とする
（ステータストークンなど機械可読な識別子は英語のまま）。
**タイトル内の固有名・技術用語・製品名の英語併記は許可**する
（例: `Playground v1`、`Semantic Type Spine`、`Share URL`）。
英語のみの長文タイトルは避け、日本語の主題を先に置くことを推奨する。

`PROPOSED` の日付メタデータは **`提案日:`** を使う。採択時に **`決定日:`** を追加する
（`決定日` は ACCEPTED 以降の採択記録）。

## 決定

### 1. 許可するステータス

主系列（各 ADR にちょうど 1 つ）:

| ステータス | 意味 |
|------------|------|
| `PROPOSED` | 提案中。採択前。本文は候補判断として書く。 |
| `ACCEPTED` | 採択済み。現行の有効な決定。 |
| `SUPERSEDED` | 後継 ADR により置換済み。`後継: ADR-NNN` を必須とする。 |

補助の終端状態（主系列と排他。いずれも許可）:

| ステータス | 意味 |
|------------|------|
| `REJECTED` | 提案を却下。後継は不要。 |
| `DEFERRED` | 判断を保留。再開条件を本文に書く。 |

**禁止する旧別名**（正規ステータストークンとして使ってはならない）:

- `DECIDED` → `ACCEPTED` を使う
- `DRAFT` → `PROPOSED` を使う
- `SURVEY` → ADR のステータスではない。調査は `docs/adr/` 外へ移すか、狭い `ACCEPTED` 決定へ変換する
- 自由記述の進捗ダッシュボード（PR 進捗・fixture 件数など）→ issues / `current-state` / RFC に置く

正規ヘッダ形式:

```text
ステータス: **ACCEPTED** — <一行の決定要約>
```

### 2. 識別子の規則

- ADR 番号は恒久識別子である。別決定に番号を再利用してはならない。
- 同一番号の本文ファイルは、廃止記録（tombstone）によるリダイレクトを除き 1 つまで。
- ファイル名: `ADR-NNN-kebab-slug.md`（番号はちょうど 3 桁のゼロ埋め。`ADR-0001` のような 4 桁以上は禁止）。
- 欠番は許容する。欠番埋めのために既存 ADR を改番してはならない。
- 削除・統合した ADR は**廃止記録**ファイルを残し、次を記録する:
  - 元タイトル
  - ステータス `SUPERSEDED`（または `REJECTED`）
  - 後継 / 吸収先（ある場合）
  - 削除・統合日と一行の理由

### 3. 後継関係（supersession）

- `廃止: ADR-NNN` / `後継: ADR-NNN` は、存在する ADR ファイル（本文または廃止記録）を指すこと。
- `SUPERSEDED` の後継は **`ACCEPTED` でなければならない**。
  `PROPOSED` の提案は現行決定を置換できない。
- 同一ファイル内での決定撤回は、`ACCEPTED` の日付付き改訂としてのみ許容する。
  後継 ADR なしに `SUPERSEDED` としてはならない。
  方針を「保留」へ切り替える場合も、**新しい ACCEPTED ADR**（保留そのものを採択）を作り、
  旧 ADR を `SUPERSEDED` にする。同一ファイルを `DEFERRED` へ書き換えて履歴を消してはならない。

### 3b. 歴史的リダイレクト

誤番号・旧パス互換のためのリダイレクトは `docs/adr/historical-redirect-*.md` に置き、
**ADR 番号を名乗らない**。`ADR-*.md` として台帳・索引に載せない。

### 4. 日付

- 決定日・改訂日はコミット日以前であること（未来日付禁止）。
- 予定作業は「予定」「issue リンク」など明示し、未来の「改訂」「完了」日付を既成事実として書いてはならない。

### 5. ADR に書くもの / 書かないもの

書く: 文脈、**理想形としての決定**、却下した代替案、帰結、再検討条件、関連リンク。

**暫定制限を決定にしない。** 「当面は禁止」「型パラメータは N 個まで」のような
実装都合の上限は、言語の理想を狭める決定として ADR に固定しない。
現行の未実装・一時制限は `docs/current-state.md`・`docs/plans/`・issues に置き、
ADR 本文では理想形を正とし、ギャップは「実装状態」として参照するにとどめる。

生きた状態として書かない: fixture 件数、PR レーン進捗、ランタイム版ピン、
週次で変わるフェーズチェックリスト。置き場は次のとおり。

| 内容 | 置き場 |
|------|--------|
| 作業追跡・完了条件 | `issues/open/` / `issues/done/` |
| 詳細仕様・長文設計 | [`docs/rfcs/`](../rfcs/) |
| 実装フェーズ・PR 計画・一時制限 | [`docs/plans/`](../plans/) |
| 現行挙動・件数・対応状況・いま効いている制限 | [`docs/current-state.md`](../current-state.md) |
| 分類しきれない設計メモ | [`docs/design/`](../design/) |

### 6. 索引と CI

- `docs/adr/README.md` は生成物であり、ステータス別に ADR を分组する
  （採択 / 提案 / 後継済み / 保留 / 却下）。
- `scripts/check/check-adrs.py` が ID 一意性・許可ステータス・未来日付禁止・
  後継先の存在を検査する。

## 帰結

- 既存 ADR の別名（`DECIDED` / `DRAFT` / `SURVEY`）は上表へ移行する。
- 調査・巨大 epic・実装フェーズ表が `docs/adr/` に残っている場合は、
  **追跡 issue を起票し、2026年7月末までに** `docs/research/`・`docs/rfcs/`・
  `docs/plans/`・`docs/current-state.md` へ移す。猶予は期限付きであり、
  ADR-000 違反の恒久的な自己正当化には使わない。
  書き方は [`docs/rfcs/000-writing-rfcs.md`](../rfcs/000-writing-rfcs.md) と
  [`docs/plans/000-writing-plans.md`](../plans/000-writing-plans.md) を参照。
- 検証ゲートは `ACCEPTED` を決定済み状態として扱う（旧 `DECIDED` は CI で拒否）。
- `PROPOSED` の本文見出しは `## 提案する決定` とし、採択済みと誤認させない。

## 参照

- 監査修復計画（2026-07）: RFC / 現状分離の前に ADR 台帳を修復する
- [`docs/rfcs/`](../rfcs/) — 詳細設計提案
- [`docs/plans/`](../plans/) — 実装計画
- `scripts/check/check-adrs.py`
- `scripts/gen/generate-docs.py`（ADR README 生成）
