...
- 根拠:
  ...
  - LLM フレンドリ (ADR-004 の方針を継承 — 解決規則が透明)
...
- 手続き: ADR-016 (Breaking Change Process) に従う
  - 各削除対象に `[breaking]` issue を作成
  - `docs/migration/v0.1-to-v0.2.md` に統合移行ガイドを作成
  - provisional 機能は deprecation period 不要 (ADR-016 準拠)
- 根拠:
...
- ADR-004 の LLM フレンドリ方針に反する (解決規則が不透明)
...
- [ ] `docs/migration/v0.1-to-v0.2.md` — 統合移行ガイド (688-697 完了時)
- [ ] 各モノモルフィック API 削除に対する `[breaking]` issue 作成
- [ ] ADR-004 (trait strategy) に「v0 post-688 update」セクション追加
