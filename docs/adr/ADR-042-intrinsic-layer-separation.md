...
検証は 3 層に分ける。`[validation]` は Python スキーマ checker が実施する不変条件のリストである。
...
`[validation]` の各 bool は checker の on/off ではなく、schema 不変条件の宣言である。checker はこれらを常に検査し、未定義・false・未知 key はエラーとする。
...
- `check_effect_lowering_consistency` — `lowering.kind` と `effect` が矛盾しない（Python 側は軽微な整合を、詳細は compiler-aware validator）。
