...
- 特定ターゲット命令との 1 対 1 対応を**明示的に**提供する（`std::wasm` 経由の raw SIMD 等）
...
- **portable SIMD**: `std::simd`（lane 型モジュール、`f32x4::add` 等）
- **raw / Wasm-specific**: `std::wasm`（`v128.load`、`v128.bitselect`、`reinterpret` 等）
