...
Component Model 対応を実現するにあたり、core Wasm モジュールを
...
当初の計画では `wasm-tools component new` を外部 subprocess として使用し（一時的委譲）、
後に in-tree 実装へ移行する予定だった。Component Model 仕様が Wasm 3.0 で
