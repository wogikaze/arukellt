...
（ADR-045: 旧 LLVM 方針は撤回済み。native 意味論・ABI は未確定。）
...
製品経路として採用する（検証記録: `docs/research/target-runtime-verification.md`）。
...
### `native-cpp` / `native-llvm` — ネイティブバックエンド（scaffold）

| 項目 | 内容 |
|------|------|
| バックエンド | C++ (`native-cpp`) または LLVM IR (`native-llvm`) |
| プラットフォーム | Linux / Windows / macOS（試験） |
| ABI / FFI / 意味論 | **未決定**（[ADR-045](ADR-045-llvm-scope-withdrawn.md)） |
| 主な用途 | ローカル実験・デバッグ（scaffold） |

native 系統は scaffold として存在する。ABI、言語意味論、FFI、安定出力形式は
ADR-045 の再評価条件を満たし後継 ADR が採択されるまで固定しない。
試験実装の現状は `docs/current-state.md` を参照する。

`wasm32-gc` は値表現（Wasm GC）・既定 host（WASI P2）・Component 出力を
**製品上の主ターゲット**として束ねる。host API 自体は ADR-011 の `std::host::*`
抽象のままである。`--host none` 等の細粒度分離は将来の改訂対象（#760）。
...
```
実装優先度: wasm32-gc（メイン） → wasm32（AtCoder維持） → native（scaffold）
言語意味論の基準: wasm32-gc
ADR-002 GC採用: wasm32-gc が正。wasm32 は同一意味の linear lowering
ADR-045 native: 未決定（再開は後継 ADR）
ADR-006 ABI: wasm32-gc = Layer 2。Layer 3 native は予約
```
...
| `native-cpp` | （scaffold） | 試験出力 | 形式・ABI 未決定（ADR-045）。現状は current-state |
...
出力形式・リンク手順・ABI は **未決定**（ADR-045）。試験実装の現状のみ
`docs/current-state.md` に記録する。本 ADR では C99 / C ABI / clang リンクを
契約として固定しない。
...
- ADR-045: 旧 LLVM 方針の撤回（native 意味論・ABI は未決定）
- ADR-006: 公開 ABI 3層構造（Layer 3 native は予約）
- ADR-008: `--emit component` は in-tree
