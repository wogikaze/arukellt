...
| 環境 | WASI | 備考 |
|------|------|------|
| Chrome | なし | ブラウザは WASI を全く用意していない。jco で変換する |
| Node.js | WASI P1 (`node:wasi`) | おまけ扱い。stdin 非対応問題あり（後述） |
| wasmtime | WASI P2/P3 | フル機能。サーバーサイド実行の主戦場 |
...
Node.js の `node:wasi` は WASI P1 に対応しているが、stdin の扱いに制限がある。
ブラウザが WASI を用意していないため、ブラウザ向けは jco transpile で
ESM + JS glue に変換する。Node.js 実行はあくまでおまけであり、
stdin を必要とするプログラムの Node.js 実行は制約を受ける場合がある。
