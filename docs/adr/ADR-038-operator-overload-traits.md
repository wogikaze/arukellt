...
| `a[i] = v` | `IndexMut` | `index_mut(self, idx) -> &mut T` | |
| `*a` | `Deref` | `deref(self) -> &Target` | |
| `&a` | `DerefMut` | `deref_mut(self) -> &mut Target` | |
...
Arukellt には借用 (`&T`) がないため、`Index::index` は値返し (`-> T`) とする。
...
generic trait (`Index<Idx, Output>`) は将来の generic trait サポート導入後に
移行する。それまでユーザー定義コレクションで `i32` 以外のキーや値を使う場合は
具象メソッド呼び出し (`c.index(i)`) で明示的に呼ぶ必要がある。

### D4: `Deref` の戻り値型

借用がないため、`Deref::deref` は値返し (`-> Target`) とする。`*a` は
`Deref::deref(a)` に脱糖する。`DerefMut` は `deref_mut(self) -> Target` で、
`*a = v` は `Target` 型の setter を経由する（または `DerefMut` を省略して
v0 では `Deref` のみ提供する）。

**v0 の制約**: D3 と同様に v0 は `Target` 型パラメータを扱えないため、
モノモーフィック署名 `fn deref(self: Deref) -> i32` を提供する。
generic `Deref<Target>` は将来の generic trait サポート後に移行する。
...
v0 では以下の trait 定義 + スカラー impl を提供する:

**定義 + スカラー impl:**
- `Add`, `Sub`, `Mul`, `Div`, `Rem`, `Neg` — 算術演算子（i32, i64, f64）
- `BitAnd`, `BitOr`, `BitXor`, `Shl`, `Shr` — ビット演算子（i32）
- `Not` — 論理否定（i32, bool）

**定義のみ（impl は将来 issue に切り出し）:**
- `Index`, `IndexMut` — インデックスアクセス（ユーザー定義型用）
- `Deref`, `DerefMut` — デリファレンス（ユーザー定義型用）

ビット演算子・シフト演算子のスカラー impl は組み込み演算子への自明な
委譲 (`self & rhs` 等) なので v0 に含める。`Index`/`IndexMut`/`Deref`/
`DerefMut` はユーザー定義型向けの trait であり、スカラー impl を提供しない
（組み込み `Vec` の `[]` アクセスは既存の `get`/`set` builtin のまま）。
...
- [x] `std/core/ops.ark` — 演算子 trait 定義 + スカラー impl
- [ ] typechecker: 演算子 → trait 解決ロジック（フォールバック付き）
- [ ] parser: `a[i] = v` / `*a` 構文の AST 拡張
- [ ] Fixture: `struct Vec2` に `impl Add for Vec2` で `v1 + v2`
- [ ] Fixture: ユーザーコレクション型に `impl Index` で `c[i]`
