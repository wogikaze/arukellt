...
`IndexMut::index_mut` はミュータブル参照の代わりに setter パターンを使用する:
...
v0 では以下の trait のみ実装する:
- `Add`, `Sub`, `Mul`, `Div`, `Neg` （算術演算子）
- `Index`, `IndexMut` （インデックスアクセス）
- `Deref` （デリファレンス）

`BitAnd`, `BitOr`, `BitXor`, `Shl`, `Shr`, `Not`, `DerefMut` は定義のみで
impl は将来issueに切り出す。
...
- [ ] `std/core/ops.ark` — 演算子 trait 定義 + スカラー impl
