...
ステータス: **ACCEPTED** — Phase 1 (構文・型チェック・ディスカバリ・カバレッジ採用 #715) 完了、Phase 2 (実行モデル) 未実装
...
Phase 1 では CLI フラグ `--filter` / `--layer` は未実装。

### D8: カバレッジ採用方針（#715）

Phase 1 の構文・CLI ディスカバリに加え、in-file テストの採用目標を定める:

| 領域 | 目標 | 方針 |
|------|------|------|
| `std/` | ≥180 `test` 宣言 | 純関数・不変条件（core, collections, text, bytes） |
| `src/compiler/` | ≥60 `test` 宣言 | 変換パス内の純ヘルパ（lexer, parser, resolver, typechecker, mir, diagnostics） |
| 副作用モジュール | fixture のみ | host, component, lsp, dap, wasm emitter 本体は in-file 対象外 |

in-file テストは **白箱ユニット**（実装近接・非 `pub` 参照可）。既存 fixture ハーネス（run / t3-compile / t3-run）は **結合・副作用** 向けと併用する。採用進捗は `python3 scripts/check/check-infile-test-adoption.py` で報告（advisory、非ブロッキング）。詳細は `docs/current-state.md` を参照。
