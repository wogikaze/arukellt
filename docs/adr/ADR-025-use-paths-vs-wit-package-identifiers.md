...
2. **キーワード分離**
   ...
   - **`import`** — レガシーファイル import（v4 で退役予定）。ADR-009 に従い退役後は Layer C 用に予約。
...
|-------|---------|-----------------------------|
| v3 | `use` + レガシー `import foo`（兄弟モジュール） | WIT ID は `.wit` / ツールに現れる |
| v4 | `import <single_identifier>` を廃止し `use` に（ADR-009） | `import` を Layer C 宣言に再利用 |
| 以降 | 既存フィクスチャは `::` パスを維持 | 新構文は additive |
