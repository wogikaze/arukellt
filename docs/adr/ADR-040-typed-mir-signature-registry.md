...
**効果**: 同じ意味型 `Point` に対して、文脈に応じて
`{type_id: Point, repr: GcRef, nullability: Nullable}` と
`{type_id: Point, repr: GcRef, nullability: NonNull}` を作れる。
型同一性・generic substitution・trait impl lookup は `TypeId` で判定し、
nullability は emit 時に `MirValueType` から取り出す。
...
- MIR verifier が type 未設定を fail にする
- T3 validate-fail が大幅減少（41件の大部分が解消される想定）
...
### PR-4: trait/generic call の戻り値型取得を registry へ切り替え

1. trait/generic call の戻り値型取得のみ registry へ切り替え
2. **この段階で41件のうちかなり動くはず**
...
PR-3はCALLのfunc_idのみ。PR-4で初めてemit経路に触れる。
