...
- `quality report` は巨大な生値の積を避け、決定的な
  `(1 + log1p(complexity)) × (1 + log1p(churn)) × (1 + centrality)` を使う。
  `centrality = log1p(fan-in) + log1p(fan-out)` の実用近似であり、churn が取得不能なら
  unknown として 0 を代入する。この順位はレビュー候補であってコード品質点数ではない。
