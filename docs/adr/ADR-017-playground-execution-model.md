...
1. **ブラウザ実行は `wasm32-gc` component パイプライン経由。**
   ...
   （GC 含む経路の検証: `docs/research/target-runtime-verification.md`）。
...
- **WASI 非依存**（ブラウザで安全に実行可能）
- **安定した構造化 API**（UI が依存できる契約）
- 実装言語・バンドル形式は本 ADR の契約ではない（一時実装は current-state / plan）

### 実行モデル: **client-side hybrid**（v1 にサーバー側 executor なし）

| 面 | 実行場所 | v1? |
|----|----------|-----|
| Edit（Monaco/CodeMirror shell） | browser | ✅ yes |
| Format / Parse / Check / Diagnostics | browser（frontend component） | ✅ yes |
| Examples（キュレート集合） | static / browser | ✅ yes |
| Share / permalink | browser + static host | ✅ yes |
...
現行の v1 frontend バンドルは Rust crate を `wasm32-unknown-unknown` に載せている。
将来 selfhost frontend へ置換しても、上記の理想契約が変わらなければ本 ADR は改訂不要。
実装の現状は `docs/current-state.md` / playground plan を正本とする。

---

## 帰結

1. **Issue 379**（ブラウザ向け Wasm パッケージ）は frontend component 契約向けに進めてよい。
...
v1 の検証は frontend component の契約（parse/format/typecheck/diagnostics）が
ブラウザで動くこと。実装言語固有のテスト配置は current-state / issue 379。
playground 固有の最小ゲート:

| 層 | 範囲 |
|----|------|
| Frontend API smoke | バンドルを import して parse/check を呼べること |
| Docs-consistency | `check-docs-consistency.py` が通ること |
| Browser smoke（v1 ゲート） | 最小 JS/HTML。フル統合は v2 |
...
コンパイラ Wasm をブラウザで実行し、コンパイル結果を jco 経由でブラウザ実行する。

### Two-stage browser pipeline

1. **Compile stage** — コンパイラ Wasm を Web Worker で実行し、
   in-memory host 経由でコンパイルする
...
- `docs/research/target-runtime-verification.md` — jco + GC 検証
- `docs/current-state.md` — 現行 frontend バンドル実装
