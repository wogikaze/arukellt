...
- 標準ライブラリは `std::prelude`, `std::core`, `std::text`, `std::bytes`, `std::collections`, `std::seq`, `std::io`, `std::time`, `std::process`, `std::wasm`, `std::wit`, `std::component`, `std::test` を中核に再構成する。
- WASI host API は `std::wasi::http` のように capability で名前付けし、`std::wasi::p1` / `std::wasi::p2` のような version namespace は導入しない。
...
また host-specific API は 2 つに分ける。portable な標準機能は `std::fs`, `std::io`, `std::env`, `std::process` に置き、WASI に強く依存する capability は `std::wasi::<capability>` に置く。WIT / Component 境界はさらに別層で、`import "wasi:..."` に委ねる。

### モジュール一覧

| モジュール | 役割 | 主要内容 |
|---|---|---|
| std::prelude | 最小限の自動 import | Option, Result, String, Bytes, Vec, Some/None, Ok/Err, panic, assert, println |
...
| std::io / fs / path | 入出力・ファイルシステム | reader, writer, file, dir, path, temp, stdin/out/err |
| std::time / std::random | 時刻・期間・乱数 | instant, duration, clock, seedable rng |
| std::process / std::env / std::cli | 実行環境 | args, env vars, exit, status, command |
| std::wasi::<capability> | target-gated host API | cli, filesystem, clocks, random, http, sockets |
...
| 層 | 例 | 目的 |
|---|---|---|
| portable stdlib | `std::fs`, `std::io`, `std::path`, `std::env`, `std::process` | 可能な限り T1/T3 で共有する surface |
| target-gated host API | `std::wasi::cli`, `std::wasi::filesystem`, `std::wasi::http`, `std::wasi::sockets` | host capability を explicit に扱う |
| component boundary | `import "wasi:cli/stdin@0.2.10"` | WIT / world / interface 宣言 |

この設計により、`std::wasi::http` は T3/T5 系専用 module として自然に定義できる。T1 で使えない capability は compile-time error にすればよく、`std::wasi::p1::http` のような不自然な namespace を増やさずに済む。
...
| 区分 | 内容 |
|---|---|
| 含める | Option, Result, String, Bytes, Vec, Some, None, Ok, Err, panic, assert, println |
...
ここでは capability model を将来に残しつつ、まずは user-facing surface を整える。`fs::read_to_string`, `fs::write`, `path::join`, `process::args` のような直感的 API を先に固定し、内部で handle/capability に lowering する設計がよい。

ただし将来の `wasi:http` や `wasi:sockets` は、この portable 層に混ぜない。これらは `std::wasi::http`, `std::wasi::sockets` のような target-gated module に置き、T1 では使えないことを明示する方がよい。
...
| std::fs | read, read_to_string, write, write_bytes, exists, create_dir_all, metadata |
| std::io | reader, writer, copy, read_exact, write_all, stdin/out/err |
| std::time | now, instant, duration, sleep(optional target-gated) |
| std::random | rng, seedable rng, next_u32, fill_bytes |
| std::env | var, vars, current_dir, set_var(target-gated) |
| std::process | args, exit, command(optional target-gated), status |

### 10.1.1 `std::*` と `std::wasi::*` の分担

- `std::fs`, `std::io`, `std::path`, `std::env`, `std::process` は portable subset を提供する
- `std::wasi::cli`, `std::wasi::filesystem` は WASI native capability を直接見せる facade とする
- `std::wasi::http`, `std::wasi::sockets` は T3/T5 系専用とし、T1 には載せない
...
ここで重要なのは、`std::wasi::*` と `import "wasi:..."` を混同しないことである。前者は Arukellt ソースから使う host API facade、後者は WIT interface / world を直接宣言する component boundary である。
...
| std::wasi::cli / filesystem / http / sockets | target-gated WASI host capability facade |
...
| `fs_read_file(path)` | `fs::read_to_string(path::from_string(path))` |
| `random_i32()` | `random::global().next_i32()` |
