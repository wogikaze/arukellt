...
These files are produced by `scripts/generate-docs.py` from `std/manifest.toml` as the
...
std/manifest.toml                        (canonical API definition)
        │
        ├─► scripts/generate-docs.py ──► docs/stdlib/reference.md
...
| # | Check | Type | Command |
|---|-------|------|---------|
| 1.1 | Generated docs match manifest | **CI** | `python3 scripts/generate-docs.py --check` |
...
python3 scripts/check-docs-consistency.py    # Gates 1–7 (18 checks)
bash scripts/verify-harness.sh --quick       # Includes docs consistency + harness checks

# If generated source inputs changed:
python3 scripts/generate-docs.py             # Regenerate all generated files
...
**Description:** `scoreboard.md` is generated by `scripts/generate-scoreboard.sh`,
