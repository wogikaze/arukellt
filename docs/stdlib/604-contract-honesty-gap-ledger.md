...
|---|---|---|---|---|
| `std::host::fs` | Filesystem helper facade | `exists`/`is_readable_file` are read-probes; `is_dir` always false; `read_dir`/`metadata` return honest Err until intrinsics land | `std/host/fs.ark`, `tests/fixtures/stdlib_fs/host_capability_contract.ark` | Keep probe semantics; directory/metadata are explicit stubs (#637) |
