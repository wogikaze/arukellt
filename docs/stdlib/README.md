...
- Manifest-backed public functions: **595**
...
- Categories: `assert` 5, `box` 2, `bytes` 45, `cli` 6, `collections` 168, `component` 2, `control` 1, `conversion` 10, `core` 22, `csv` 7, `env` 6, `fs` 3, `host_clock` 2, `host_env` 5, `host_fs` 7, `host_http` 2, `host_process` 2, `host_random` 4, `host_sockets` 1, `host_stdio` 4, `host_udp` 1, `io` 28, `json` 22, `math` 5, `numeric` 2, `option_result` 14, `path` 9, `process` 2, `random` 3, `scalar_conversion` 14, `seq` 13, `signal` 11, `string` 20, `test` 17, `text` 42, `time` 3, `toml` 8, `wasm` 61, `wit` 16
...
| [613-result-surface-inventory.md](613-result-surface-inventory.md) | Issue: issues/open/613-errhandle-stdlib-result-surface.md (not yet filed) |
...
| [scoreboard.md](scoreboard.md) | Hand-maintained today (auto-generator not yet implemented). |
