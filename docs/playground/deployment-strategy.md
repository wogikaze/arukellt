...
> - Wasm content-hash cache busting via `scripts/gen/stamp-playground-assets.sh` (issue #437 closeout): when `docs/playground/wasm/ark_playground_wasm_bg.wasm` exists, the script writes `ark_playground_wasm_bg-<hash>.wasm` plus `asset-manifest.json`, and `docs/playground/index.html` loads the hashed Wasm URL from that manifest.
>
