 ...
 */
// ---------------------------------------------------------------------------
// Telemetry switch (compile-time constant)
// ---------------------------------------------------------------------------
/**
 ...
 */
export const TELEMETRY_DISABLED = true;
// ---------------------------------------------------------------------------
// Error reporting (browser-local only in v1)
// ---------------------------------------------------------------------------
/**
...
export function reportError(context, error) {
    // v1: local console only.  No outbound requests.
    // v2+: extend here per privacy-telemetry-policy.md §4.2 — only when
    //       TELEMETRY_DISABLED is replaced by a runtime opt-in preference.
    console.error(`[ark-playground] ${context}`, error);
}
/**
 ...
 */
export function reportWasmLoadError(error) {
    reportError("Wasm load failed", error);
    return ("The compiler could not be loaded. " +
 ...
 * @returns A user-facing error message string (never null).
 */
export function reportCompilerPanic(error) {
    reportError("Compiler panic", error);
    return ("The compiler encountered an unexpected error. " +
        "Please try again or report a bug on GitHub.");
}
