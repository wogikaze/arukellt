 ...
 * ```
 */
export function createDiagnosticsPanel(container, options = {}) {
    ...
    return {
        ...
        update(diagnostics, source) {
            ...
            if (helpCount > 0)
                summaryParts.push(`${helpCount} note${helpCount !== 1 ? "s" : ""}`);
 ...
 * ```
 */
export function buildDiagnosticOverlay(source, diagnostics) {
    ...
    const charSev = new Array(source.length).fill(null);
 ...
 */
export function injectDiagnosticStyles(css) {
