...
describe("buildDiagnosticOverlay", () => {
    ...
    it("wraps error range in a marker span", () => {
        ...
        assert.ok(result.includes('<span class="ark-diag-marker-error">123</span>'), `Expected error marker span, got: ${result}`);
        // Text before the marker should be plain
        assert.ok(result.startsWith("fn "), `Expected plain text before marker, got: ${result}`);
    });
    it("wraps warning range in a warning marker span", () => {
        ...
        const result = buildDiagnosticOverlay(source, [diag]);
        assert.ok(result.includes('<span class="ark-diag-marker-warning">_x</span>'), `Expected warning marker span, got: ${result}`);
    });
    it("wraps help range in a help marker span", () => {
        ...
        const result = buildDiagnosticOverlay(source, [diag]);
        assert.ok(result.includes('<span class="ark-diag-marker-help">x</span>'), `Expected help marker span, got: ${result}`);
    });
    it("handles multiple non-overlapping diagnostics", () => {
        ...
        assert.ok(result.includes('<span class="ark-diag-marker-error">bad</span>'));
        assert.ok(result.includes('<span class="ark-diag-marker-warning">bad2</span>'));
    });
    it("highest severity wins for overlapping ranges", () => {
        ...
        assert.ok(result.includes('<span class="ark-diag-marker-warning">b</span>'), `Expected warning marker for 'b', got: ${result}`);
        assert.ok(result.includes('<span class="ark-diag-marker-error">cd</span>'), `Expected error marker for 'cd', got: ${result}`);
        assert.ok(result.includes('<span class="ark-diag-marker-warning">e</span>'), `Expected warning marker for 'e', got: ${result}`);
    });
    it("escapes HTML in source text", () => {
        ...
        const result = buildDiagnosticOverlay(source, [diag]);
        assert.ok(result.includes("&lt;&gt;"), "Expected HTML-escaped angle brackets");
    ...
    it("clamps label ranges to source bounds", () => {
        ...
        const result = buildDiagnosticOverlay(source, [diag]);
        assert.ok(result.includes('<span class="ark-diag-marker-error">bc</span>'), `Expected clamped range marker, got: ${result}`);
    });
    ...
    it("handles multiple labels in a single diagnostic", () => {
        ...
        const result = buildDiagnosticOverlay(source, [diag]);
        assert.ok(result.includes('<span class="ark-diag-marker-error">a</span>'));
        assert.ok(result.includes('<span class="ark-diag-marker-error">b</span>'));
    });
});
