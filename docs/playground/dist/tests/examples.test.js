...
import { EXAMPLES, FIXTURE_BASE_PATH, getExample, getExampleList, getExamplesByTag, getFixtureMap, } from "../examples.js";
// ---------------------------------------------------------------------------
// Catalog structure
// ---------------------------------------------------------------------------
describe("Examples catalog structure", () => {
    ...
    it("all examples have required fields", () => {
        for (const ex of EXAMPLES) {
            ...
            assert.ok(typeof ex.description === "string" && ex.description.length > 0, `description for ${ex.id}`);
            assert.ok(typeof ex.source === "string" && ex.source.length > 0, `source for ${ex.id}`);
        }
    });
    ...
    it("all IDs are kebab-case", () => {
        ...
        for (const ex of EXAMPLES) {
            assert.ok(kebabRe.test(ex.id), `ID "${ex.id}" should be kebab-case`);
        }
    });
    it("all sources end with newline", () => {
        for (const ex of EXAMPLES) {
            assert.ok(ex.source.endsWith("\n"), `Source for "${ex.id}" should end with newline`);
        }
    });
        ...
        for (const ex of EXAMPLES) {
            if (ex.tags !== undefined) {
                ...
                for (const tag of ex.tags) {
                    assert.ok(typeof tag === "string" && tag.length > 0, `tag in ${ex.id} should be non-empty string`);
                }
            }
        }
    });
    ...
    it("includes all tagged examples", () => {
        ...
        for (const ex of EXAMPLES) {
            if (ex.tags && ex.tags.length > 0) {
                assert.ok(allTaggedIds.has(ex.id), `${ex.id} should appear in tag map`);
            }
        }
    });
});
...
describe("Fixture references", () => {
    it("every example has a fixturePath", () => {
        for (const ex of EXAMPLES) {
            assert.ok(typeof ex.fixturePath === "string" && ex.fixturePath.length > 0, `${ex.id} should have a non-empty fixturePath`);
        }
    });
    it("all fixture paths end with .ark", () => {
        for (const ex of EXAMPLES) {
            assert.ok(ex.fixturePath.endsWith(".ark"), `fixturePath for "${ex.id}" should end with .ark, got "${ex.fixturePath}"`);
        }
    });
    it("all fixture paths are unique", () => {
        const paths = EXAMPLES.map((e) => e.fixturePath);
        const unique = new Set(paths);
        assert.equal(unique.size, paths.length, "duplicate fixture paths detected");
    });
    it("fixture paths do not start with /", () => {
        for (const ex of EXAMPLES) {
            assert.ok(!ex.fixturePath.startsWith("/"), `fixturePath for "${ex.id}" should be relative, got "${ex.fixturePath}"`);
        }
    });
...
describe("getFixtureMap", () => {
    ...
    it("maps example IDs to fixture paths", () => {
        const map = getFixtureMap();
        for (const ex of EXAMPLES) {
            assert.equal(map.get(ex.id), ex.fixturePath, `fixture map for "${ex.id}" should match fixturePath`);
        }
    });
