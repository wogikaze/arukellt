...
- On `wasm32-gc` with WASI P2, the emitter imports `wasi:cli/*` and related Preview 2 interface names directly ([issue 510](../../issues/done/510-t3-p2-import-table-switch.md)). Bridged in-tree emit ([issue 714](../../issues/done/714-wasi-p2-emitter-native-component-output.md)) produces a `wasi:cli/command` component without a P1 adapter: component imports `wasi:cli/stdout` + `wasi:io/streams`, stdout is adapted through host `get-stdout` + `blocking-write-and-flush`, and `gate_074` proves `wasm-tools validate` + wasmtime prints `hello p2` ([issue 074](../../issues/done/074-wasi-p2-native-component.md)). Stderr is bridged the same way (`wasi:cli/stderr` + `get-stderr`;
  `gate-668-p2-stderr`). Guest-native `get-stdout` call sites and args/env
  (GC environment lowering) remain [#668](../../issues/open/668-p2-native-component-polish.md). Canonical scratch limits: [`docs/plans/component-canonical-memory.md`](../plans/component-canonical-memory.md).
